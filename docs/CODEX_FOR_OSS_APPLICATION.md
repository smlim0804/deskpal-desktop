...
```text
현재 공개 버전은 클라우드 AI 없이 동작합니다. 캐릭터는 데스크톱 위에서 돌아다니며 바로가기와 시스템 상태/벤치마킹 알림을 제공합니다. 설정은 로컬에 저장하고 Electron contextIsolation을 사용합니다. 장기적으로는 캐릭터별 성격과 사용자가 선택한 AI 제공자를 연결해 더 똑똑한 데스크톱 동료로 발전시키고 싶습니다. 그래서 Codex의 코드리뷰, 보안 점검, 테스트 보강이 프로젝트에 잘 맞습니다.
```
