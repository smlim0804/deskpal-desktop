...
function personalityLikeText(personality) {
  return (personality?.likes || []).map((actionId) => careText(actionId)).join(" · ");
}
...
function totalEffectEquips(care) {
  return Math.round(clamp(Number(care?.actionCounts?.effectEquip) || 0, 0, 999999));
}
...
function snackVarietyCount(game) {
  return Object.values(snackInventoryFor(game)).filter((count) => count > 0).length;
}
...
function petAlbumRecord(game, characterId) {
  return petAlbumFor(game)[characterId] || null;
}
...
function charmCraftCount(care, charmId) {
  return Math.round(clamp(Number(care?.actionCounts?.[`charm:${charmId}`]) || 0, 0, 999999));
}
...
function totalMoodMoments(game) {
  const moments = moodMomentsFor(game);
  return Object.values(moments.counts).reduce((sum, count) => sum + count, 0);
}
...
function totalTalentTraining(care) {
  return Math.round(clamp(Number(care?.actionCounts?.talentTrain) || 0, 0, 999999));
}
...
function duoMoveCount(care, moveId) {
  return Math.round(clamp(Number(care?.actionCounts?.[`duo:${moveId}`]) || 0, 0, 999999));
}
...
function careIdleLine(pet) {
  const care = careForPet(pet);
  const mood = careMood(care);
  const language = currentLanguage();
  if (language === "ko") {
    if (mood === "hungry") return "배고파.";
    if (mood === "sleepy") return "졸려.";
    if (mood === "messy") return "씻고 싶어.";
    if (mood === "lonely") return "심심해.";
    if (mood === "bright") return "좋아.";
    return "여기 있어.";
  }
  if (mood === "hungry") return "Hungry.";
  if (mood === "sleepy") return "Sleepy.";
  if (mood === "messy") return "Needs a clean.";
  if (mood === "lonely") return "Bored.";
  if (mood === "bright") return "Good.";
  return "Here.";
}
...
function activeDesktopObjectForPet(pet, now = performance.now()) {
  if (!pet) return null;
  return desktopObjects.find((object) => (
    object
    && !object.claimed
    && object.ownerSlot === pet.slotIndex
    && now - object.born < object.life
  )) || null;
}
...
function performSimpleCareAction(pet, actionId) {
  const action = CARE_ACTIONS[actionId];
  const slot = settings?.slots?.[pet?.slotIndex];
  if (!action || !slot) return;
  const care = careForPet(pet, { decay: true });
  for (const key of ["hunger", "happiness", "energy", "hygiene", "training"]) {
    if (!Number.isFinite(action[key])) continue;
    care[key] = Math.round(clamp(care[key] + action[key], 0, 100));
  }
  care.bond = Math.round(clamp(care.bond + (action.bond || 0), 0, 9999));
  const leveled = addCareXp(care, action.xp || 0);
  care.actionCounts[actionId] = (care.actionCounts[actionId] || 0) + 1;
  care.lastActionAt = Date.now();
  care.lastCareAt = Date.now();
  care.combo = { lastAction: actionId, lastAt: care.lastActionAt, chain: 1 };
  slot.care = normalizeCare(care);
  pet.care = slot.care;
  ...
  pet.vy = 0;
  spawnCareBurst(pet, actionId);
  showPetThought(pet, simpleCareLine(actionId, slot.care, leveled), { durationMs: 2600 });
  api.updateSettings(settings).then((next) => {
    settings = next;
    syncPets();
    if (activePet === pet && !panel.hidden) openPanel(pet, { noBurst: true, view: "care" });
  });
}
...
function maybePetInteractions(now) {
  if (!settings?.enabled || activePet) return;
  for (let i = 0; i < pets.length; i += 1) {
    const a = pets[i];
    if (!a.enabled || a.dragging || a.pausedByPanel || now - (a.lastSocialAt || 0) < 9000) continue;
    for (let j = i + 1; j < pets.length; j += 1) {
      const b = pets[j];
      if (!b.enabled || b.dragging || b.pausedByPanel || now - (b.lastSocialAt || 0) < 9000) continue;
      const ax = a.x + getPetSize(a) / 2;
      const ay = a.y + getPetSize(a) / 2;
      const bx = b.x + getPetSize(b) / 2;
      const by = b.y + getPetSize(b) / 2;
      const distance = Math.hypot(bx - ax, by - ay);
      const threshold = (getPetSize(a) + getPetSize(b)) * 0.52;
      if (distance > threshold || Math.random() > 0.018) continue;
      triggerPetInteraction(a, b, now);
      return;
    }
  }
}

function maybePetDiscoveries(now) {
  if (!settings?.enabled || activePet || !INTERACTION_VARIETY_READY) return;
  for (const pet of pets) {
    if (!pet.enabled || pet.dragging || pet.pausedByPanel) continue;
    if (!pet.nextDiscoveryAt) pet.nextDiscoveryAt = now + rand(22000, 48000);
    if (now < pet.nextDiscoveryAt) continue;
    pet.nextDiscoveryAt = now + rand(36000, 86000);
    const care = pet.care || careForPet(pet);
    const mood = careMood(care);
    const chance = mood === "bright" ? 0.52 : mood === "calm" ? 0.34 : mood === "lonely" ? 0.24 : 0.16;
    if (Math.random() > chance) continue;
    triggerDiscovery(pet);
    return;
  }
}

function maybePetAmbientEvents(now) {
  if (!settings?.enabled || activePet) return;
  const game = ensureGame();
  const ambient = normalizeAmbientEvents(game.ambientEvents);
  if (Date.now() - ambient.lastAt < 36000) return;
  for (const pet of pets) {
    if (!pet.enabled || pet.dragging || pet.pausedByPanel) continue;
    if (!pet.nextAmbientAt) pet.nextAmbientAt = now + rand(30000, 76000);
    if (now < pet.nextAmbientAt) continue;
    pet.nextAmbientAt = now + rand(90000, 180000);
    const care = pet.care || careForPet(pet);
    const mood = careMood(care);
    const chance = mood === "bright" ? 0.42 : mood === "calm" ? 0.32 : mood === "sleepy" ? 0.22 : 0.18;
    if (Math.random() > chance) continue;
    triggerAmbientEvent(pet, now);
    return;
  }
}

function maybeCareQuirkReactions(now) {
  if (!settings?.enabled || activePet) return;
  for (const pet of pets) {
    if (!pet.enabled || pet.dragging || pet.pausedByPanel || pet.thoughtEl) continue;
    if (!pet.nextQuirkAt) pet.nextQuirkAt = now + rand(85000, 180000);
    if (now < pet.nextQuirkAt) continue;
    pet.nextQuirkAt = now + rand(160000, 340000);
    const care = pet.care || careForPet(pet);
    const quirks = activeCareQuirks(care);
    if (!quirks.length || care.energy < 18) continue;
    const effects = careQuirkEffects(care);
    const comboEffects = careQuirkComboEffects(care);
    if (Math.random() > clamp(0.1 + effects.idle + comboEffects.idle * 0.5, 0.12, 0.72)) continue;
    const combos = activeCareQuirkCombos(care);
    if (combos.length && Math.random() < 0.42) {
      const combo = combos[Math.floor(Math.random() * combos.length)];
      const kick = 1 + effects.motion + comboEffects.motion;
      pet.vx += rand(-1.05, 1.05) * kick;
      pet.vy += rand(-0.8, 0.8) * kick;
      pet.spinVelocity += rand(-6, 6) * kick;
      spawnCareQuirkComboBurst(pet, combo.id, now, combo);
      showPetThought(pet, careQuirkComboIdleLine(pet, combo), { durationMs: 3800 });
      return;
    }
    const quirk = quirks[Math.floor(Math.random() * quirks.length)];
    const kick = 1 + effects.motion;
    pet.vx += rand(-0.9, 0.9) * kick;
    pet.vy += rand(-0.7, 0.7) * kick;
    pet.spinVelocity += rand(-5, 5) * kick;
    spawnCareQuirkBurst(pet, quirk.id, now, quirk);
    showPetThought(pet, careQuirkIdleLine(pet, quirk), { durationMs: 3600 });
    return;
  }
}

function maybeAutoMicroEvents(now) {
  if (!settings?.enabled || activePet || now - lastAutoMicroEventAt < 65000) return;
  for (const pet of pets) {
    if (!pet.enabled || pet.dragging || pet.pausedByPanel) continue;
    if (!pet.nextMicroEventAt) pet.nextMicroEventAt = now + rand(70000, 150000);
    if (now < pet.nextMicroEventAt) continue;
    const care = pet.care || careForPet(pet);
    const recentlyHandled = Date.now() - (Number(care.lastActionAt) || 0) < 42000;
    const mood = careMood(care);
    const chance = mood === "bright" ? 0.34 : mood === "calm" ? 0.26 : mood === "lonely" ? 0.2 : 0.16;
    pet.nextMicroEventAt = now + rand(150000, 360000);
    if (recentlyHandled || care.energy < 24 || Math.random() > chance) continue;
    const event = microEventOptions(pet, care, settings?.game || null)[0]?.event;
    if (!event) continue;
    if (triggerAutoMicroEvent(pet, event.id)) {
      lastAutoMicroEventAt = now;
      return;
    }
  }
}
...
function makeCareMeter(labelText, value) {
  ...
  wrap.className = "care-meter";
  const top = document.createElement("div");
  top.className = "care-meter__top";
  const label = document.createElement("span");
  ...
  bar.style.setProperty("--value", `${clamp(value, 0, 100)}%`);
  wrap.append(top, bar);
  return wrap;
}

function renderCarePanel(pet, character) {
  const care = careForPet(pet, { decay: true });
  const card = document.createElement("section");
  card.className = "care-card";
  card.dataset.mood = careMood(care);

  const head = document.createElement("div");
  head.className = "care-head";
  const title = document.createElement("strong");
  title.textContent = careText("careTitle");
  const badges = document.createElement("div");
  badges.className = "care-badges";
  const level = document.createElement("span");
  level.textContent = `${careText("level")} ${care.level}`;
  const stage = document.createElement("span");
  stage.textContent = careText(careStage(care));
  const mood = document.createElement("span");
  mood.textContent = `${careText("mood")} ${careText(careMood(care))}`;
  badges.append(level, stage, mood);
  head.append(title, badges);

  const xpWrap = document.createElement("div");
  xpWrap.className = "care-xp";
  const xpTop = document.createElement("div");
  xpTop.className = "care-meter__top";
  const xpLabel = document.createElement("span");
  xpLabel.textContent = careText("xp");
  const xpNumber = document.createElement("strong");
  const threshold = careThreshold(care.level);
  xpNumber.textContent = `${care.xp}/${threshold}`;
  xpTop.append(xpLabel, xpNumber);
  const xpBar = document.createElement("i");
  xpBar.style.setProperty("--value", `${clamp((care.xp / threshold) * 100, 0, 100)}%`);
  xpWrap.append(xpTop, xpBar);

  const meters = document.createElement("div");
  meters.className = "care-meters";
  meters.append(
    makeCareMeter(careText("hunger"), care.hunger),
    makeCareMeter(careText("happiness"), care.happiness),
    makeCareMeter(careText("energy"), care.energy),
    makeCareMeter(careText("hygiene"), care.hygiene),
    makeCareMeter(careText("training"), care.training),
    makeCareMeter(careText("bond"), Math.min(100, care.bond)),
  );

  const actions = document.createElement("div");
  actions.className = "care-actions";
  for (const actionId of CARE_ACTION_IDS) {
    const action = CARE_ACTIONS[actionId];
    const button = document.createElement("button");
    button.className = "care-action";
    button.type = "button";
    button.title = careText(actionId);
    const icon = document.createElement("span");
    icon.textContent = action.icon;
    const text = document.createElement("strong");
    text.textContent = careText(actionId);
    button.append(icon, text);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      performCareAction(pet, actionId);
    });
    actions.appendChild(button);
  }

  card.append(head, xpWrap, meters, actions);
  return card;
}

function renderCareCoachPanel(pet, character) {
  const care = careForPet(pet);
  const suggestions = careCoachSuggestions(pet, care, character);
  const card = document.createElement("section");
  card.className = "care-coach-card";

  const head = document.createElement("div");
  head.className = "care-coach-head";
  const title = document.createElement("strong");
  title.textContent = careText("careCoachTitle");
  const count = document.createElement("span");
  count.textContent = careText("careCoachReady");
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "care-coach-grid";
  for (const suggestion of suggestions) {
    const action = CARE_ACTIONS[suggestion.actionId];
    if (!action) continue;
    const button = document.createElement("button");
    button.className = "care-coach-button";
    button.type = "button";
    button.style.setProperty("--coach-color", suggestion.color);
    button.title = `${careText(suggestion.labelKey)} · ${careText(suggestion.descKey)} · ${careText(suggestion.actionId)}`;

    const icon = document.createElement("span");
    icon.className = "care-coach-icon";
    icon.textContent = suggestion.icon;

    const info = document.createElement("span");
    info.className = "care-coach-info";
    const name = document.createElement("strong");
    name.textContent = careText(suggestion.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(suggestion.descKey);
    ...
    actionChip.className = "care-coach-action";
    actionChip.textContent = `+ ${careText(suggestion.actionId)}`;

    button.append(icon, info, actionChip);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      performCareAction(pet, suggestion.actionId);
    });
...
function renderMicroEventPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const options = microEventOptions(pet, care, game);
  const card = document.createElement("section");
  card.className = "micro-event-card";

  const head = document.createElement("div");
  head.className = "micro-event-head";
  const title = document.createElement("strong");
  title.textContent = careText("microEventTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("microEventPatterns")} ${formatCompactCount(microEventVarietyCount())}`;
  head.append(title, count);

  const auto = document.createElement("p");
  auto.className = "micro-event-auto";
  auto.textContent = `${careText("microEventAuto")} ${Math.round(care.actionCounts?.microEventAuto || 0)} · ${careText("microEventAutoDesc")}`;

  const grid = document.createElement("div");
  grid.className = "micro-event-grid";
  for (const { event, score } of options) {
    const reward = microEventReward(pet, care, game, event);
    const ready = care.energy >= event.energyCost + 2;
    const button = document.createElement("button");
    button.className = "micro-event-button";
    button.classList.toggle("is-locked", !ready);
    button.type = "button";
    button.style.setProperty("--micro-color", event.color);
    button.title = ready
      ? `${careText(event.labelKey)} · ${roomRewardText(reward)}`
      : `${careText("microEventNeedEnergy")} ${care.energy}/${event.energyCost + 2}`;

    const icon = document.createElement("span");
    icon.className = "micro-event-icon";
    icon.textContent = event.icon;

    const info = document.createElement("span");
    info.className = "micro-event-info";
    const name = document.createElement("strong");
    name.textContent = careText(event.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(event.descKey);
    const meta = document.createElement("em");
    meta.textContent = ready
      ? `${careText("microEventReward")} ${roomRewardText(reward)}`
      : `${careText("microEventNeedEnergy")} ${care.energy}/${event.energyCost + 2}`;
    const scoreLine = document.createElement("em");
    scoreLine.textContent = `${careText("microEventDone")} ${microEventCount(care, event.id)} · ${careText("growthScore")} ${score}`;
    info.append(name, desc, meta, scoreLine);

    const action = document.createElement("span");
    action.className = "micro-event-action";
    action.textContent = ready ? `+ ${careText("microEventRun")}` : careText("microEventNeedEnergy");

    button.append(icon, info, action);
    button.addEventListener("click", (eventObject) => {
      eventObject.preventDefault();
      eventObject.stopPropagation();
      performMicroEvent(pet, event.id);
    });
    grid.appendChild(button);
  }

  card.append(head, auto, grid);
  return card;
}

function renderPetWalkPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const card = document.createElement("section");
  ...
  const title = document.createElement("strong");
  title.textContent = careText("walkTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("walkTimes")} ${totalPetWalks(care)}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "walk-grid";
  for (const walkId of PET_WALK_IDS) {
    const walk = { id: walkId, ...PET_WALKS[walkId] };
    const readiness = petWalkReady(pet, walk, care);
    const reward = petWalkReward(pet, walk, care, game);
    const button = document.createElement("button");
    button.className = "walk-button";
    button.classList.toggle("is-locked", !readiness.ok);
    button.type = "button";
    button.style.setProperty("--walk-color", walk.color);
    button.title = readiness.ok
      ? `${careText(walk.labelKey)} · ${petWalkRewardText(reward)}`
      : readiness.reason === "level"
        ? `${careText("walkNeedLevel")} ${walk.minLevel}`
        : `${careText("walkNeedEnergy")} ${care.energy}/${walk.energyCost}`;

    const icon = document.createElement("span");
    icon.className = "walk-icon";
    icon.textContent = walk.icon;

    const info = document.createElement("span");
    info.className = "walk-info";
    const name = document.createElement("strong");
    name.textContent = careText(walk.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(walk.descKey);
    const meta = document.createElement("em");
    meta.textContent = readiness.ok
      ? `${careText("walkNeedEnergy")} -${walk.energyCost} · ${petWalkBonusText(pet, care, game, walk)}`
      : readiness.reason === "level"
        ? `${careText("walkNeedLevel")} ${care.level}/${walk.minLevel}`
        : `${careText("walkNeedEnergy")} ${care.energy}/${walk.energyCost}`;
    const rewardLine = document.createElement("em");
    rewardLine.textContent = `${careText("walkReward")} ${petWalkRewardText(reward)}`;
    info.append(name, desc, meta, rewardLine);

    const action = document.createElement("span");
    action.className = "walk-action";
    action.textContent = readiness.ok
      ? `+ ${careText("walkRun")}`
      : readiness.reason === "level"
        ? careText("walkNeedLevel")
        : careText("walkNeedEnergy");

    button.append(icon, info, action);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      performPetWalk(pet, walkId);
    });
    grid.appendChild(button);
  }
...
function renderTrainingYardPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const card = document.createElement("section");
  ...
  const head = document.createElement("div");
  head.className = "yard-head";
  const title = document.createElement("strong");
  title.textContent = careText("yardTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("yardTimes")} ${totalTrainingYardRuns(care)}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "yard-grid";
  for (const courseId of TRAINING_YARD_COURSE_IDS) {
    const course = { id: courseId, ...TRAINING_YARD_COURSES[courseId] };
    const readiness = trainingYardReady(pet, course, care);
    const reward = trainingYardReward(pet, course, care, game);
    const button = document.createElement("button");
    button.className = "yard-button";
    button.classList.toggle("is-locked", !readiness.ok);
    button.type = "button";
    button.style.setProperty("--yard-color", course.color);
    button.title = readiness.ok
      ? `${careText(course.labelKey)} · ${trainingYardRewardText(reward)}`
      : readiness.reason === "level"
        ? `${careText("yardNeedLevel")} ${course.minLevel}`
        : `${careText("yardNeedEnergy")} ${care.energy}/${course.energyCost}`;

    const icon = document.createElement("span");
    icon.className = "yard-icon";
    icon.textContent = course.icon;

    const info = document.createElement("span");
    info.className = "yard-info";
    const name = document.createElement("strong");
    name.textContent = careText(course.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(course.descKey);
    const meta = document.createElement("em");
    meta.textContent = readiness.ok
      ? `${careText("yardNeedEnergy")} -${course.energyCost} · ${trainingYardBonusText(pet, care, game, course)}`
      : readiness.reason === "level"
        ? `${careText("yardNeedLevel")} ${care.level}/${course.minLevel}`
        : `${careText("yardNeedEnergy")} ${care.energy}/${course.energyCost}`;
    const rewardLine = document.createElement("em");
    rewardLine.textContent = `${careText("yardReward")} ${trainingYardRewardText(reward)}`;
    info.append(name, desc, meta, rewardLine);

    const action = document.createElement("span");
    action.className = "yard-action";
    action.textContent = readiness.ok
      ? `+ ${careText("yardRun")}`
      : readiness.reason === "level"
        ? careText("yardNeedLevel")
        : careText("yardNeedEnergy");

    button.append(icon, info, action);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      performTrainingYard(pet, courseId);
    });
    grid.appendChild(button);
  }
...
function renderPatrolPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const card = document.createElement("section");
  ...
  const head = document.createElement("div");
  head.className = "patrol-head";
  const title = document.createElement("strong");
  title.textContent = careText("patrolTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("patrolTimes")} ${totalPatrolRuns(care)}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "patrol-grid";
  for (const routeId of PATROL_ROUTE_IDS) {
    const route = { id: routeId, ...PATROL_ROUTES[routeId] };
    const readiness = patrolReady(pet, route, care);
    const reward = patrolReward(pet, route, care, game);
    const button = document.createElement("button");
    button.className = "patrol-button";
    button.classList.toggle("is-locked", !readiness.ok);
    button.type = "button";
    button.style.setProperty("--patrol-color", route.color);
    button.title = readiness.ok
      ? `${careText(route.labelKey)} · ${patrolRewardText(reward)}`
      : readiness.reason === "level"
        ? `${careText("patrolNeedLevel")} ${route.minLevel}`
        : `${careText("patrolNeedEnergy")} ${care.energy}/${route.energyCost}`;

    const icon = document.createElement("span");
    icon.className = "patrol-icon";
    icon.textContent = route.icon;

    const info = document.createElement("span");
    info.className = "patrol-info";
    const name = document.createElement("strong");
    name.textContent = careText(route.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(route.descKey);
    const meta = document.createElement("em");
    meta.textContent = readiness.ok
      ? `${careText("patrolNeedEnergy")} -${route.energyCost} · ${patrolBonusText(pet, care, game, route)}`
      : readiness.reason === "level"
        ? `${careText("patrolNeedLevel")} ${care.level}/${route.minLevel}`
        : `${careText("patrolNeedEnergy")} ${care.energy}/${route.energyCost}`;
    const rewardLine = document.createElement("em");
    rewardLine.textContent = `${careText("patrolReward")} ${patrolRewardText(reward)}`;
    info.append(name, desc, meta, rewardLine);

    const action = document.createElement("span");
    action.className = "patrol-action";
    action.textContent = readiness.ok
      ? `+ ${careText("patrolRun")}`
      : readiness.reason === "level"
        ? careText("patrolNeedLevel")
        : careText("patrolNeedEnergy");

    button.append(icon, info, action);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      performPatrolRoute(pet, routeId);
    });
    grid.appendChild(button);
  }
...
function renderEvolutionPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const current = evolutionCurrentForm(care, game);
  const card = document.createElement("section");
  card.className = "evolution-card";
  card.style.setProperty("--evolution-color", current.color);

  const head = document.createElement("div");
  head.className = "evolution-head";
  const title = document.createElement("strong");
  title.textContent = careText("evolutionTitle");
  const count = document.createElement("span");
  count.textContent = `${totalEvolutionForms(care)}/${EVOLUTION_FORM_IDS.length}`;
  head.append(title, count);

  const currentBox = document.createElement("div");
  currentBox.className = "evolution-current";
  ...
  currentName.textContent = `${careText("evolutionCurrent")} · ${careText(current.labelKey)}`;
  const currentDesc = document.createElement("span");
  currentDesc.textContent = careText(current.descKey);
  currentInfo.append(currentName, currentDesc);
  currentBox.append(currentIcon, currentInfo);

  const grid = document.createElement("div");
  grid.className = "evolution-grid";
  for (const formId of EVOLUTION_FORM_IDS) {
    const form = { id: formId, ...EVOLUTION_FORMS[formId] };
    const unlocked = evolutionFormUnlocked(care, game, form);
    const claimed = evolutionFormClaimed(care, formId);
    const button = document.createElement("button");
    button.className = "evolution-button";
    button.classList.toggle("is-locked", !unlocked);
    button.classList.toggle("is-claimed", claimed);
    button.type = "button";
    button.style.setProperty("--form-color", form.color);
    button.title = unlocked
      ? `${careText(form.descKey)} · ${careText("evolutionReward")} ${evolutionRewardText(form.reward)}`
      : `${careText("evolutionNeed")} ${evolutionRequirementText(care, game, form)}`;

    const icon = document.createElement("span");
    icon.className = "evolution-icon";
    icon.textContent = form.icon;

    const info = document.createElement("span");
    info.className = "evolution-info";
    const name = document.createElement("strong");
    name.textContent = careText(form.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(form.descKey);
    const meta = document.createElement("em");
    meta.textContent = unlocked
      ? `${careText("evolutionReward")} ${evolutionRewardText(form.reward)}`
      : `${careText("evolutionNeed")} ${evolutionRequirementText(care, game, form)}`;
    info.append(name, desc, meta);

    const action = document.createElement("span");
    action.className = "evolution-action";
    action.textContent = claimed ? careText("evolutionClaimed") : unlocked ? `+ ${careText("evolutionClaim")}` : careText("evolutionLocked");

    button.append(icon, info, action);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      claimEvolutionForm(pet, formId);
    });
    grid.appendChild(button);
  }

  card.append(head, currentBox, grid);
  return card;
}

function renderMoodPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const moodId = careMood(care);
  const aura = moodAuraFor(moodId);
  ...
  card.style.setProperty("--mood-color", aura.color);

  const head = document.createElement("div");
  head.className = "mood-head";
  const title = document.createElement("strong");
  title.textContent = careText("moodAuraTitle");
  const state = document.createElement("span");
  state.textContent = careText(moodId);
  ...
  icon.textContent = aura.icon;
  const info = document.createElement("div");
  info.className = "mood-info";
  const name = document.createElement("strong");
  name.textContent = careText(aura.labelKey);
  const desc = document.createElement("span");
  desc.textContent = careText(aura.descKey);
  info.append(name, desc);
  body.append(icon, info);

  const facts = document.createElement("div");
  facts.className = "mood-facts";
  const seen = document.createElement("span");
  seen.textContent = `${careText("moodAuraSeen")} ${totalMoodChecks(care)} · ${moodMomentCount(game, moodId)}`;
  const rewardText = document.createElement("span");
  rewardText.textContent = `${careText("moodAuraReward")} ${moodAuraRewardText(reward)}`;
  facts.append(seen, rewardText);

  const button = document.createElement("button");
  button.className = "mood-button";
  button.type = "button";
  button.textContent = `+ ${careText("moodAuraCheck")}`;
  button.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    performMoodCheck(pet);
  });

  card.append(head, body, facts, button);
  return card;
}

function renderRequestPanel(pet) {
  const care = careForPet(pet);
  const request = ensureCareRequest(pet, care);
  const action = CARE_ACTIONS[request.action] || CARE_ACTIONS.pet;
  const bonus = requestBonusFor(care, request.action);
  const card = document.createElement("section");
  card.className = "request-card";
  card.classList.toggle("is-done", request.done);

  const head = document.createElement("div");
  head.className = "request-head";
  const title = document.createElement("strong");
  title.textContent = careText("requestTitle");
  const state = document.createElement("span");
  state.textContent = request.done ? careText("requestDone") : careText(request.action);
  head.append(title, state);
  ...
  const name = document.createElement("strong");
  name.textContent = careText(request.action);
  const reason = document.createElement("span");
  reason.textContent = requestReasonLine(pet, care, request.action);
  ...
  reasonLabel.textContent = `${careText("requestReason")} · ${careText(careMood(care))}`;
  const rewardLabel = document.createElement("span");
  rewardLabel.textContent = `${careText("requestBonus")} · ${requestBonusText(bonus)}`;
  meta.append(reasonLabel, rewardLabel);

  const button = document.createElement("button");
  button.className = "request-button";
  button.type = "button";
  button.disabled = request.done;
  button.textContent = request.done ? careText("requestDone") : `+ ${careText("requestDo")}`;
  button.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    performCareRequest(pet);
  });

  card.append(head, body, meta, button);
  return card;
}

function renderComboPanel(pet) {
  const care = careForPet(pet);
  const comboState = normalizeCareCombo(care.combo);
  const nextCombos = nextCareCombos(care);
  const shownCombos = nextCombos.length
    ? nextCombos
    : CARE_COMBO_IDS.slice(0, 3).map((id) => ({ id, ...CARE_COMBOS[id] }));
  const card = document.createElement("section");
  card.className = "combo-card";
  card.classList.toggle("is-ready", nextCombos.length > 0);

  const head = document.createElement("div");
  head.className = "combo-head";
  const title = document.createElement("strong");
  title.textContent = careText("comboTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("comboChain")} ${comboState.chain || 0}`;
  head.append(title, count);

  const intro = document.createElement("p");
  intro.className = "combo-intro";
  intro.textContent = nextCombos.length
    ? `${careText("comboReady")} · ${careText(comboState.lastAction)} → ${shownCombos.map((combo) => careText(combo.to)).join(", ")}`
    : careText("comboStart");
  ...
  for (const combo of shownCombos) {
    ...
    item.classList.toggle("is-next", combo.from === comboState.lastAction);
    const icon = document.createElement("span");
    ...
    const name = document.createElement("strong");
    name.textContent = careText(combo.labelKey);
    const path = document.createElement("span");
    path.textContent = `${careText(combo.from)} → ${careText(combo.to)} · ${careComboRewardText(combo)}`;
    const desc = document.createElement("em");
    desc.textContent = careText(combo.descKey);
    info.append(name, path, desc);
...
function renderRoutinePanel(pet) {
  const care = careForPet(pet);
  const state = normalizeCareRoutine(care.routine);
  const today = currentDayKey();
  const card = document.createElement("section");
  ...
  const title = document.createElement("strong");
  title.textContent = careText("routineTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("done")} ${totalCareRoutines(care)}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "routine-grid";
  for (const routine of CARE_ROUTINE_DEFS) {
    const active = state.id === routine.id && state.dayKey === today;
    const completed = active && state.completedToday;
    const stepIndex = active ? clamp(state.step, 0, routine.steps.length - 1) : 0;
    const nextAction = routine.steps[stepIndex] || routine.steps[0];
    const energyNeed = Math.max(0, -(CARE_ACTIONS[nextAction]?.energy || 0));
    const lowEnergy = !completed && energyNeed > 0 && care.energy < energyNeed + 3;

    const button = document.createElement("button");
    button.className = "routine-button";
    button.classList.toggle("is-active", active && !completed);
    button.classList.toggle("is-done", completed);
    button.classList.toggle("is-locked", lowEnergy);
    button.type = "button";
    button.disabled = completed;
    button.style.setProperty("--routine-color", routine.color);
    button.title = `${careText(routine.descKey)} · ${routineRewardText(routine.reward)}`;

    const icon = document.createElement("span");
    icon.className = "routine-icon";
    icon.textContent = routine.icon;

    const info = document.createElement("span");
    info.className = "routine-info";
    const name = document.createElement("strong");
    name.textContent = careText(routine.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(routine.descKey);
    const meta = document.createElement("em");
    const shownStep = completed ? routine.steps.length : Math.min(routine.steps.length, stepIndex + 1);
    meta.textContent = `${careText("routineStep")} ${shownStep}/${routine.steps.length} · ${careText("done")} ${routineCount(care, routine.id)}`;
    const steps = document.createElement("span");
    steps.className = "routine-steps";
    routine.steps.forEach((actionId, index) => {
      const chip = document.createElement("i");
      chip.className = "routine-step";
      chip.classList.toggle("is-done", active && (completed || index < state.step));
      chip.classList.toggle("is-next", active && !completed && index === state.step);
      chip.textContent = `${CARE_ACTIONS[actionId]?.icon || ""} ${careText(actionId)}`.trim();
      steps.appendChild(chip);
    });
    info.append(name, desc, meta, steps);

    const action = document.createElement("span");
    action.className = "routine-action";
    if (completed) {
      action.textContent = careText("routineDoneToday");
    } else if (lowEnergy) {
      action.textContent = careText("routineNeedEnergy");
    } else if (active) {
      action.textContent = `+ ${careText("routineDoStep")}`;
    } else {
      action.textContent = `+ ${careText("routineStart")}`;
    }
    ...
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      performRoutineStep(pet, routine.id);
    });
    grid.appendChild(button);
  }
...
function renderCommandPanel(pet) {
  const care = careForPet(pet);
  const card = document.createElement("section");
  ...
  const title = document.createElement("strong");
  title.textContent = careText("commandTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("commandTimes")} ${totalPetCommands(care)}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "command-grid";
  for (const commandId of PET_COMMAND_IDS) {
    const command = PET_COMMANDS[commandId];
    const lowEnergy = care.energy < command.energyCost + 2;
    const button = document.createElement("button");
    button.className = "command-button";
    button.classList.toggle("is-locked", lowEnergy);
    button.type = "button";
    button.style.setProperty("--command-color", command.color);
    button.title = `${careText(command.descKey)} · -${command.energyCost} ${careText("energy")} · ${petCommandRewardText(command.reward)}`;
    ...
    const name = document.createElement("strong");
    name.textContent = careText(command.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(command.descKey);
    const meta = document.createElement("em");
    meta.textContent = `${careText("commandTimes")} ${petCommandCount(care, commandId)} · -${command.energyCost} ${careText("energy")}`;
    info.append(name, desc, meta);

    const action = document.createElement("span");
    action.className = "command-action";
    action.textContent = lowEnergy ? careText("commandNeedEnergy") : `+ ${careText("commandUse")}`;

    button.append(icon, info, action);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      performPetCommand(pet, commandId);
    });
    grid.appendChild(button);
  }
...
function renderPersonalityPanel(pet) {
  const personality = personalityForPet(pet);
  const care = careForPet(pet);
  const target = MILESTONE_DEFS.find((item) => item.id === "personalityMatch")?.target || 8;
  const likedCount = personalityLikedActionCount(pet, care);
  const percent = clamp((likedCount / target) * 100, 0, 100);

  const card = document.createElement("section");
  card.className = "personality-card";

  const head = document.createElement("div");
  head.className = "personality-head";
  const title = document.createElement("strong");
  title.textContent = careText("personalityTitle");
  const badge = document.createElement("span");
  badge.textContent = careText(personality.labelKey);
  head.append(title, badge);

  const main = document.createElement("div");
  main.className = "personality-main";
  const icon = document.createElement("span");
  icon.className = "personality-icon";
  icon.textContent = personality.icon;
  const info = document.createElement("div");
  info.className = "personality-info";
  const name = document.createElement("strong");
  name.textContent = careText(personality.labelKey);
  const desc = document.createElement("span");
  desc.textContent = careText(personality.descKey);
  info.append(name, desc);
  main.append(icon, info);

  const likes = document.createElement("div");
  likes.className = "personality-likes";
  const likesLabel = document.createElement("span");
  likesLabel.className = "personality-likes__label";
  likesLabel.textContent = careText("personalityLikes");
  likes.appendChild(likesLabel);
  for (const actionId of personality.likes || []) {
    const chip = document.createElement("span");
    chip.className = "personality-like";
    const bonus = personalityCareBonus(personality, actionId);
    const bonusText = bonus
      ? Object.entries(bonus)
          .map(([key, value]) => `+${value} ${careText(key)}`)
          .join(", ")
      : careText("personalityBonus");
    chip.title = bonusText;
    chip.textContent = `${CARE_ACTIONS[actionId]?.icon || ""} ${careText(actionId)}`.trim();
    likes.appendChild(chip);
  }

  const progress = document.createElement("div");
  progress.className = "personality-progress";
  const progressTop = document.createElement("div");
  progressTop.className = "personality-progress__top";
  const progressLabel = document.createElement("span");
  progressLabel.textContent = careText("personalityProgress");
  const progressValue = document.createElement("strong");
  progressValue.textContent = `${Math.min(likedCount, target)}/${target}`;
  ...
  progress.append(progressTop, progressBar);

  card.append(head, main, likes, progress);
  return card;
}

function renderGrowthPathPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const path = growthPathFor(care, game);
  const card = document.createElement("section");
  card.className = "growth-card";
  card.style.setProperty("--growth-color", path.def.color);

  const head = document.createElement("div");
  head.className = "growth-head";
  const title = document.createElement("strong");
  title.textContent = careText("growthPathTitle");
  const state = document.createElement("span");
  state.textContent = `${careText(path.rankKey)} · ${path.score}`;
  head.append(title, state);
  ...
  icon.textContent = path.def.icon;
  const info = document.createElement("div");
  info.className = "growth-info";
  const name = document.createElement("strong");
  name.textContent = `${careText("growthCurrent")} · ${careText(path.def.labelKey)}`;
  const desc = document.createElement("span");
  desc.textContent = careText(path.def.descKey);
  const bonus = document.createElement("em");
  bonus.textContent = `${careText("growthBonus")} ${growthPathBonusText(path)}`;
  info.append(name, desc, bonus);
  main.append(icon, info);

  const grid = document.createElement("div");
  grid.className = "growth-grid";
  for (const pathId of GROWTH_PATH_IDS) {
    const def = GROWTH_PATHS[pathId];
    const score = path.scores[pathId] || 0;
    const next = score >= 150 ? 150 : score >= 80 ? 150 : 80;
    const item = document.createElement("div");
    item.className = "growth-item";
    item.classList.toggle("is-active", pathId === path.id);
    item.style.setProperty("--growth-color", def.color);

    const top = document.createElement("div");
    top.className = "growth-item__top";
    const label = document.createElement("strong");
    label.textContent = `${def.icon} ${careText(def.labelKey)}`;
    const value = document.createElement("span");
    value.textContent = `${score}/${next}`;
    top.append(label, value);

    const bar = document.createElement("i");
    bar.style.setProperty("--value", `${clamp((score / Math.max(1, next)) * 100, 0, 100)}%`);
    item.append(top, bar);
    grid.appendChild(item);
  }

  card.append(head, main, grid);
  return card;
}

function renderGrowthRewardPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const claimed = claimedGrowthRewardSet(care);
  const card = document.createElement("section");
  card.className = "growth-reward-card";

  const head = document.createElement("div");
  head.className = "growth-reward-head";
  const title = document.createElement("strong");
  title.textContent = careText("growthRewardsTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("growthRewardsClaimed")} ${claimed.size}/${GROWTH_REWARD_IDS.length}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "growth-reward-grid";
  for (const rewardId of GROWTH_REWARD_IDS) {
    const reward = { id: rewardId, ...GROWTH_REWARDS[rewardId] };
    const progress = growthRewardProgress(reward, pet, care, game);
    const isClaimed = claimed.has(rewardId);
    const isReady = progress.done && !isClaimed;
    const item = document.createElement("article");
    item.className = "growth-reward-item";
    item.classList.toggle("is-ready", isReady);
    item.classList.toggle("is-claimed", isClaimed);
    item.style.setProperty("--growth-reward-color", reward.color);

    const icon = document.createElement("span");
    icon.className = "growth-reward-icon";
    icon.textContent = reward.icon;

    const info = document.createElement("div");
    info.className = "growth-reward-info";
    const name = document.createElement("strong");
    name.textContent = careText(reward.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(reward.descKey);
    const meta = document.createElement("em");
    meta.textContent = `${Math.min(progress.value, progress.target)}/${progress.target} · ${careText("growthRewardReward")} ${growthRewardText(reward)}`;
    const unlock = document.createElement("em");
    unlock.textContent = `${careText("growthRewardUnlock")} ${growthRewardEffectText(reward)}`;
    const bar = document.createElement("i");
    bar.style.setProperty("--value", `${progress.percent}%`);
    info.append(name, desc, meta, unlock, bar);

    const button = document.createElement("button");
    button.className = "growth-reward-button";
    button.type = "button";
    button.disabled = !isReady;
    button.textContent = isClaimed
      ? careText("growthRewardClaimed")
      : isReady
        ? careText("growthRewardClaim")
        : careText("locked");
    button.title = isReady
      ? `${careText("growthRewardClaim")} · ${growthRewardText(reward)}`
      : `${careText("growthRewardNeed")} ${Math.min(progress.value, progress.target)}/${progress.target}`;
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      claimGrowthReward(pet, reward.id);
    });
...
function renderCareQuirkPanel(pet) {
  const care = careForPet(pet);
  const activeCount = totalCareQuirks(care);
  const card = document.createElement("section");
  card.className = "quirk-card";

  const head = document.createElement("div");
  head.className = "quirk-head";
  const title = document.createElement("strong");
  title.textContent = careText("quirksTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("quirksActive")} ${activeCount}/${CARE_QUIRK_IDS.length}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "quirk-grid";
  for (const quirkId of CARE_QUIRK_IDS) {
    const quirk = { id: quirkId, ...CARE_QUIRKS[quirkId] };
    const progress = careQuirkProgress(care, quirk);
    const item = document.createElement("article");
    item.className = "quirk-item";
    item.classList.toggle("is-locked", !progress.done);
    item.style.setProperty("--quirk-color", quirk.color);

    const icon = document.createElement("span");
    icon.className = "quirk-icon";
    icon.textContent = quirk.icon;

    const info = document.createElement("div");
    info.className = "quirk-info";
    const name = document.createElement("strong");
    name.textContent = careText(quirk.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(quirk.descKey);
    const need = document.createElement("em");
    need.textContent = progress.done
      ? careText("done")
      : `${careText("quirkNeed")} ${Math.min(progress.value, progress.target)}/${progress.target}`;
    const effect = document.createElement("em");
    effect.textContent = `${careText("quirkEffect")} ${careQuirkEffectText(quirk)}`;
    const meter = document.createElement("i");
    meter.style.setProperty("--value", `${progress.percent}%`);
    info.append(name, desc, need, effect, meter);
...
function renderCareQuirkComboPanel(pet) {
  const care = careForPet(pet);
  const activeCount = totalCareQuirkCombos(care);
  const card = document.createElement("section");
  card.className = "quirk-combo-card";

  const head = document.createElement("div");
  head.className = "quirk-combo-head";
  const title = document.createElement("strong");
  title.textContent = careText("quirkCombosTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("quirkCombosActive")} ${activeCount}/${CARE_QUIRK_COMBO_IDS.length}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "quirk-combo-grid";
  for (const comboId of CARE_QUIRK_COMBO_IDS) {
    const combo = { id: comboId, ...CARE_QUIRK_COMBOS[comboId] };
    const unlocked = careQuirkComboUnlocked(care, combo);
    const item = document.createElement("article");
    item.className = "quirk-combo-item";
    item.classList.toggle("is-locked", !unlocked);
    item.style.setProperty("--quirk-combo-color", combo.color);

    const icon = document.createElement("span");
    icon.className = "quirk-combo-icon";
    icon.textContent = combo.icon;

    const info = document.createElement("div");
    info.className = "quirk-combo-info";
    const name = document.createElement("strong");
    name.textContent = careText(combo.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(combo.descKey);
    const need = document.createElement("em");
    need.textContent = unlocked
      ? careText("done")
      : `${careText("quirkComboNeed")} ${careQuirkComboNeedText(combo)}`;
    const effect = document.createElement("em");
    effect.textContent = `${careText("quirkComboEffect")} ${careQuirkComboEffectText(combo)}`;
    info.append(name, desc, need, effect);
...
function renderMoodPatternPanel(pet) {
  const care = careForPet(pet);
  const activeCount = totalMoodPatterns(care);
  const card = document.createElement("section");
  card.className = "mood-pattern-card";

  const head = document.createElement("div");
  head.className = "mood-pattern-head";
  const title = document.createElement("strong");
  title.textContent = careText("moodPatternsTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("moodPatternsActive")} ${activeCount}/${MOOD_PATTERN_IDS.length}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "mood-pattern-grid";
  for (const patternId of MOOD_PATTERN_IDS) {
    const pattern = { id: patternId, ...MOOD_PATTERNS[patternId] };
    const progress = moodPatternProgress(care, pattern);
    const item = document.createElement("article");
    item.className = "mood-pattern-item";
    item.classList.toggle("is-locked", !progress.done);
    item.style.setProperty("--mood-pattern-color", pattern.color);
    ...
    const info = document.createElement("div");
    info.className = "mood-pattern-info";
    const name = document.createElement("strong");
    name.textContent = careText(pattern.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(pattern.descKey);
    const need = document.createElement("em");
    need.textContent = progress.done
      ? careText("done")
      : `${careText("moodPatternNeed")} ${Math.min(progress.value, progress.target)}/${progress.target}`;
    const effect = document.createElement("em");
    effect.textContent = `${careText("moodPatternEffect")} ${moodPatternEffectText(pattern)}`;
    const meter = document.createElement("i");
    meter.style.setProperty("--value", `${progress.percent}%`);
    info.append(name, desc, need, effect, meter);
...
function renderPetHabitPanel(pet) {
  const care = careForPet(pet);
  const activeCount = totalPetHabits(care);
  const card = document.createElement("section");
  ...
  const title = document.createElement("strong");
  title.textContent = careText("petHabitsTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("petHabitsActive")} ${activeCount}/${PET_HABIT_IDS.length}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "habit-grid";
  for (const habitId of PET_HABIT_IDS) {
    const habit = { id: habitId, ...PET_HABITS[habitId] };
    const progress = petHabitProgress(care, habit);
    const item = document.createElement("article");
    ...
    const name = document.createElement("strong");
    name.textContent = careText(habit.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(habit.descKey);
    const need = document.createElement("em");
    need.textContent = progress.done
      ? careText("done")
      : `${careText("petHabitNeed")} ${Math.min(progress.value, progress.target)}/${progress.target}`;
    const effect = document.createElement("em");
    effect.textContent = `${careText("petHabitEffect")} ${petHabitEffectText(habit)}`;
    const meter = document.createElement("i");
    meter.style.setProperty("--value", `${progress.percent}%`);
    info.append(name, desc, need, effect, meter);
...
function renderAnimalInstinctPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const progress = animalInstinctProgress(pet, care, game);
  const instinct = progress.instinct;
  ...
  const card = document.createElement("section");
  card.className = "animal-instinct-card";
  card.style.setProperty("--instinct-color", instinct.color);

  const head = document.createElement("div");
  head.className = "animal-instinct-head";
  const title = document.createElement("strong");
  title.textContent = careText("animalInstinctTitle");
  const count = document.createElement("span");
  count.textContent = progress.done ? careText("animalInstinctActive") : `${Math.min(progress.value, progress.target)}/${progress.target}`;
  head.append(title, count);

  const main = document.createElement("div");
  main.className = "animal-instinct-main";
  const icon = document.createElement("span");
  icon.className = "animal-instinct-icon";
  icon.textContent = instinct.icon;

  const info = document.createElement("div");
  info.className = "animal-instinct-info";
  const name = document.createElement("strong");
  name.textContent = careText(instinct.labelKey);
  const desc = document.createElement("span");
  desc.textContent = careText(instinct.descKey);
  const need = document.createElement("em");
  need.textContent = progress.done
    ? careText("done")
    : `${careText("animalInstinctNeed")} ${Math.min(progress.value, progress.target)}/${progress.target}`;
  const effect = document.createElement("em");
  effect.textContent = `${careText("animalInstinctEffect")} ${animalInstinctEffectText(instinct)}`;
  const meter = document.createElement("i");
  meter.style.setProperty("--value", `${progress.percent}%`);
  info.append(name, desc, need, effect, meter);
...
function renderBondPerkPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const activeCount = totalBondPerks(care, game);
  const card = document.createElement("section");
  ...
  const title = document.createElement("strong");
  title.textContent = careText("bondPerksTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("bondPerksUnlocked")} ${activeCount}/${BOND_PERK_IDS.length}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "perk-grid";
  for (const perkId of BOND_PERK_IDS) {
    const perk = { id: perkId, ...BOND_PERKS[perkId] };
    const unlocked = bondPerkUnlocked(care, game, perk);
    const item = document.createElement("article");
    item.className = "perk-item";
    item.classList.toggle("is-locked", !unlocked);
    item.style.setProperty("--perk-color", perk.color);
    ...
    const name = document.createElement("strong");
    name.textContent = careText(perk.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(perk.descKey);
    const need = document.createElement("em");
    need.textContent = unlocked
      ? careText("done")
      : `${careText("bondPerkNeed")} ${bondPerkRequirementText(care, game, perk)}`;
    const effect = document.createElement("em");
    effect.textContent = `${careText("bondPerkEffect")} ${bondPerkEffectText(perk)}`;
    info.append(name, desc, need, effect);
...
function renderCaretakerRankPanel() {
  const game = ensureGame();
  const rank = caretakerRankFor(game);
  const stats = rank.stats;
  const card = document.createElement("section");
  card.className = "caretaker-card";
  card.style.setProperty("--caretaker-color", rank.color || "#42d7c5");

  const head = document.createElement("div");
  head.className = "caretaker-head";
  const title = document.createElement("strong");
  title.textContent = careText("caretakerRankTitle");
  const score = document.createElement("span");
  score.textContent = `${careText("caretakerRankScore")} ${stats.score}`;
  head.append(title, score);

  const main = document.createElement("div");
  main.className = "caretaker-main";
  const icon = document.createElement("span");
  icon.className = "caretaker-icon";
  icon.textContent = rank.icon;
  const info = document.createElement("div");
  info.className = "caretaker-info";
  const name = document.createElement("strong");
  name.textContent = careText(rank.labelKey);
  const desc = document.createElement("span");
  desc.textContent = careText(rank.descKey);
  const bonus = document.createElement("em");
  bonus.textContent = `${careText("caretakerRankBonus")} ${caretakerRankBonusText(rank)}`;
  info.append(name, desc, bonus);
  main.append(icon, info);

  const meter = document.createElement("div");
  meter.className = "caretaker-meter";
  const meterTop = document.createElement("div");
  meterTop.className = "caretaker-meter__top";
  const nextLabel = document.createElement("span");
  nextLabel.textContent = rank.next
    ? `${careText("caretakerRankNext")} ${careText(rank.next.labelKey)}`
    : careText("caretakerRankMax");
  const nextValue = document.createElement("strong");
  nextValue.textContent = rank.next ? `${stats.score}/${rank.next.min}` : `${stats.score}`;
  meterTop.append(nextLabel, nextValue);
  const meterBar = document.createElement("i");
  meterBar.style.setProperty("--value", `${rank.percent}%`);
  meter.append(meterTop, meterBar);

  const statGrid = document.createElement("div");
  statGrid.className = "caretaker-stats";
  for (const item of [
    { key: "caretakerStatPets", value: stats.activePets },
    { key: "caretakerStatCare", value: stats.totalCare },
    { key: "caretakerStatBond", value: stats.totalBond },
    { key: "caretakerStatCollections", value: stats.collections },
    { key: "caretakerStatMilestones", value: stats.milestones },
    { key: "caretakerStatPerks", value: stats.totalPerks },
  ]) {
    const chip = document.createElement("span");
    const label = document.createElement("em");
    label.textContent = careText(item.key);
  ...
  card.append(head, main, meter, statGrid);
  return card;
}
...
function renderAmbientPanel() {
  ...
  const ambient = normalizeAmbientEvents(game.ambientEvents);
  const date = new Date();
  const season = SEASONS[currentSeasonId(date)] || SEASONS.spring;
  const time = TIME_BANDS[currentTimeBandId(date)] || TIME_BANDS.afternoon;
  const events = activeAmbientEvents(date).slice(0, 4);
  const total = totalAmbientEventCount(game);
  const lastEvent = AMBIENT_EVENTS[ambient.lastId];

  const card = document.createElement("section");
  card.className = "ambient-card";

  const head = document.createElement("div");
  head.className = "ambient-head";
  const title = document.createElement("strong");
  title.textContent = careText("ambientTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("ambientTotal")} ${total}`;
  head.append(title, count);

  const facts = document.createElement("div");
  facts.className = "ambient-facts";
  for (const item of [
    { label: careText("ambientSeason"), value: careText(season.labelKey), color: season.color },
    { label: careText("ambientTime"), value: careText(time.labelKey), color: time.color },
  ]) {
    const chip = document.createElement("span");
    chip.style.setProperty("--ambient-color", item.color);
    const label = document.createElement("em");
  ...
  const list = document.createElement("div");
  list.className = "ambient-list";
  for (const event of events) {
    const item = document.createElement("article");
    item.className = "ambient-item";
    item.style.setProperty("--ambient-color", event.color);
    const icon = document.createElement("span");
    icon.className = "ambient-icon";
    icon.textContent = event.icon;
    const info = document.createElement("div");
    info.className = "ambient-info";
    const name = document.createElement("strong");
    name.textContent = careText(event.labelKey);
    const desc = document.createElement("span");
    desc.textContent = `${careText(event.descKey)} · ${ambientRewardText(event)}`;
    info.append(name, desc);
    item.append(icon, info);
    list.appendChild(item);
  }
  ...
  if (lastEvent) {
    ...
    last.className = "ambient-last";
    last.textContent = `${careText("done")} · ${careText(lastEvent.labelKey)}`;
    card.append(head, facts, list, last);
  } else {
    card.append(head, facts, list);
  }
  return card;
}

function claimDailyStreak(pet) {
  const slot = settings?.slots?.[pet?.slotIndex];
  if (!slot) return;
  const game = ensureGame();
  const today = currentDayKey();
  if (!canClaimDailyStreak(game, today)) {
    const claimedLine =
      currentLanguage() === "ko" ? "오늘 출석 보상은 이미 받았어." : "Today's streak reward is already claimed.";
    showPetThought(pet, claimedLine, { durationMs: 3000 });
    return;
  }

  const care = careForPet(pet, { decay: true });
  const streak = dailyStreakFor(game);
  const reward = dailyStreakReward(game);
  game.coins = Math.round(clamp(game.coins + reward.coins, 0, 999999));
  if (reward.happiness) care.happiness = Math.round(clamp(care.happiness + reward.happiness, 0, 100));
  if (reward.bond) care.bond = Math.round(clamp(care.bond + reward.bond, 0, 9999));
  if (reward.xp) addCareXp(care, reward.xp);
  care.lastActionAt = Date.now();
  care.lastCareAt = Date.now();
  game.streak = { ...streak, claimedDayKey: today };
  slot.care = normalizeCare(care);
  settings.game = normalizeGame(game);
  pet.care = slot.care;

  const line = dailyStreakLine(characterFor(pet.characterId), settings.game.streak, reward);
  recordMemory(pet, "ST", line);
  spawnMilestoneBurst(pet);
  showPetThought(pet, line, { durationMs: 4400 });
  api.updateSettings(settings).then((next) => {
    settings = next;
    syncPets();
    if (activePet === pet && !panel.hidden) openPanel(pet, { noBurst: true });
...
function claimLeagueSeasonTier(pet, tierId) {
  const tier = LEAGUE_SEASON_TIERS.find((item) => item.id === tierId);
  const slot = settings?.slots?.[pet?.slotIndex];
  if (!tier || !slot) return;
  const game = ensureGame();
  const season = leagueSeasonFor(game);
  const claimed = new Set(season.claimedTiers);
  if (claimed.has(tierId)) {
    showPetThought(pet, `${careText(tier.labelKey)} · ${careText("leagueSeasonClaimed")}`, { durationMs: 3000 });
    return;
  }
  if (season.points < tier.target) {
    showPetThought(pet, `${careText("leagueSeasonLocked")} ${season.points}/${tier.target}`, { durationMs: 3200 });
    return;
  }
  ...
  care.lastActionAt = Date.now();
  care.lastCareAt = Date.now();
  season.claimedTiers = [...claimed, tierId].filter((id) => LEAGUE_SEASON_TIER_IDS.includes(id));
  game.leagueSeason = normalizeLeagueSeason(season);
  slot.care = normalizeCare(care);
  settings.game = normalizeGame(game);
  pet.care = slot.care;

  const line = leagueSeasonClaimLine(characterFor(pet.characterId), tier, reward);
  recordMemory(pet, tier.icon, line);
  spawnMilestoneBurst(pet);
  showPetThought(pet, line, { durationMs: 4400 });
  api.updateSettings(settings).then((next) => {
    settings = next;
...
function startFocusSession(pet, minutes) {
  const slot = settings?.slots?.[pet?.slotIndex];
  if (!slot) return;
  const game = ensureGame();
  const focus = focusFor(game);
  const targetMinutes = FOCUS_PRESETS.includes(minutes) ? minutes : DEFAULT_GAME.focus.targetMinutes;
  if (focus.startedAt > 0) {
    const remaining = Math.max(0, focus.targetMinutes * 60000 - focusElapsedMs(focus));
    const line = focus.activeSlot === pet.slotIndex
      ? `${careText("focusRunning")} · ${formatFocusClock(remaining)}`
      : careText("focusOtherPet");
    showPetThought(pet, line, { durationMs: 3200 });
    spawnFocusBurst(pet);
    return;
  }
  game.focus = normalizeFocus({
    ...focus,
  ...
  pet.vx *= 0.38;
  pet.vy *= 0.38;
  pet.replySlowUntil = performance.now() + 1800;
  const character = characterFor(pet.characterId);
  const line = currentLanguage() === "ko"
    ? `${localStyleFor(character.id)} ${targetMinutes}분 집중을 지켜볼게.`
    : `${localStyleFor(character.id)} I will guard ${targetMinutes}m of focus.`;
  spawnFocusBurst(pet);
  showPetThought(pet, line, { durationMs: 3600 });
  api.updateSettings(settings).then((next) => {
    settings = next;
...
function cancelFocusSession(pet) {
  const game = ensureGame();
  const focus = focusFor(game);
  if (focus.startedAt <= 0 || focus.activeSlot !== pet?.slotIndex) return;
  game.focus = normalizeFocus({ ...focus, activeSlot: -1, startedAt: 0 });
  settings.game = normalizeGame(game);
  const character = characterFor(pet.characterId);
  const line = currentLanguage() === "ko"
    ? `${localStyleFor(character.id)} 집중 기록을 취소했어. 다음에 다시 잡자.`
    : `${localStyleFor(character.id)} Focus session canceled. We can try again.`;
  showPetThought(pet, line, { durationMs: 3000 });
  api.updateSettings(settings).then((next) => {
    settings = next;
    syncPets();
    if (activePet === pet && !panel.hidden) openPanel(pet, { noBurst: true });
...
function finishFocusSession(pet) {
  const slot = settings?.slots?.[pet?.slotIndex];
  if (!slot) return;
  const game = ensureGame();
  const focus = focusFor(game);
  if (focus.startedAt <= 0 || focus.activeSlot !== pet.slotIndex) return;
  const elapsedMs = focusElapsedMs(focus);
  if (elapsedMs < FOCUS_MIN_COMPLETE_MS) {
    showPetThought(pet, `${careText("focusNeedTime")} · ${formatFocusClock(FOCUS_MIN_COMPLETE_MS - elapsedMs)}`, {
      durationMs: 3200,
    });
    spawnFocusBurst(pet);
    return;
  ...
  const minutes = Math.round(clamp(Math.max(1, Math.floor(elapsedMs / 60000)), 1, Math.max(focus.targetMinutes, focus.targetMinutes * 2)));
  const reward = focusReward(minutes, focus.targetMinutes);
  const care = careForPet(pet, { decay: true });
  const character = characterFor(pet.characterId);
  game.focus = normalizeFocus({
    ...focus,
    activeSlot: -1,
    startedAt: 0,
    totalMinutes: focus.totalMinutes + minutes,
    completed: focus.completed + 1,
    bestMinutes: Math.max(focus.bestMinutes, minutes),
    lastCompletedDayKey: currentDayKey(),
  });
  game.coins = Math.round(clamp(game.coins + reward.coins, 0, 999999));
  care.happiness = Math.round(clamp(care.happiness + reward.happiness, 0, 100));
  care.energy = Math.round(clamp(care.energy + reward.energy, 0, 100));
  care.training = Math.round(clamp(care.training + reward.training, 0, 100));
  care.bond = Math.round(clamp(care.bond + reward.bond, 0, 9999));
  care.actionCounts.focusSession = (care.actionCounts.focusSession || 0) + 1;
  care.actionCounts.focusMinutes = (care.actionCounts.focusMinutes || 0) + minutes;
  care.lastActionAt = Date.now();
  care.lastCareAt = Date.now();
  settings.game = normalizeGame(game);
  const completedQuests = updateDailyQuests("focus");
  const leveled = addCareXp(care, reward.xp);
  slot.care = normalizeCare(care);
  settings.game = normalizeGame(settings.game);
  pet.care = slot.care;
  ...
  pet.spinVelocity += rand(-8, 8);
  const levelText = leveled ? currentLanguage() === "ko" ? ` 레벨 ${slot.care.level} 달성!` : ` Reached level ${slot.care.level}!` : "";
  const line = `${focusLine(character, minutes, focus.targetMinutes, reward, completedQuests)}${levelText}`;
  recordMemory(pet, "FO", line);
  spawnFocusBurst(pet);
  spawnMilestoneBurst(pet);
  showPetThought(pet, line, { durationMs: 5600 });
  api.updateSettings(settings).then((next) => {
...
function renderStreakPanel(pet) {
  const game = ensureGame();
  const streak = dailyStreakFor(game);
  const reward = dailyStreakReward(game);
  const claimed = !canClaimDailyStreak(game);
  const card = document.createElement("section");
  card.className = "streak-card";

  const head = document.createElement("div");
  head.className = "streak-head";
  const title = document.createElement("strong");
  title.textContent = careText("dailyStreakTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("dailyStreak")} ${streak.current}`;
  head.append(title, count);

  const facts = document.createElement("div");
  facts.className = "streak-facts";
  for (const item of [
    { label: careText("dailyStreak"), value: `${streak.current}` },
    { label: careText("dailyBest"), value: `${streak.best}` },
    { label: careText("dailyStreakReward"), value: dailyStreakRewardText(reward) },
  ]) {
  ...
  button.textContent = claimed ? careText("dailyClaimed") : `+ ${careText("dailyClaim")}`;
  button.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    claimDailyStreak(pet);
  });
...
function renderFocusPanel(pet) {
  const game = ensureGame();
  const care = careForPet(pet);
  const focus = focusFor(game);
  const active = focus.startedAt > 0;
  const mine = active && focus.activeSlot === pet.slotIndex;
  const elapsedMs = focusElapsedMs(focus);
  const targetMs = Math.max(1, focus.targetMinutes * 60000);
  const remainingMs = Math.max(0, targetMs - elapsedMs);
  const percent = active ? clamp((elapsedMs / targetMs) * 100, 0, 100) : 0;
  const canBank = mine && elapsedMs >= FOCUS_MIN_COMPLETE_MS;
  ...
  const title = document.createElement("strong");
  title.textContent = careText("focusModeTitle");
  const state = document.createElement("span");
  state.textContent = active
    ? `${careText(mine ? (canBank ? "focusReady" : "focusRunning") : "focusOtherPet")} · ${formatFocusClock(remainingMs)}`
    : `${careText("focusTotal")} ${formatFocusMinutes(focus.totalMinutes)}`;
  head.append(title, state);
  ...
  intro.textContent = active
    ? mine
      ? `${formatFocusMinutes(focusElapsedMinutes(focus))}/${formatFocusMinutes(focus.targetMinutes)}`
      : careText("focusOtherPet")
    : careText("focusIdleHint");

  const meter = document.createElement("div");
  meter.className = "focus-meter";
  const fill = document.createElement("span");
  fill.style.width = `${percent}%`;
  meter.appendChild(fill);

  const facts = document.createElement("div");
  facts.className = "focus-facts";
  for (const item of [
    { label: careText("focusSessions"), value: `${totalFocusSessions(care)}` },
    { label: careText("focusTotal"), value: formatFocusMinutes(totalFocusMinutes(care)) },
    { label: careText("focusBest"), value: formatFocusMinutes(focus.bestMinutes) },
  ]) {
    const fact = document.createElement("span");
    const label = document.createElement("em");
    label.textContent = item.label;
  ...
  if (active) {
    ...
    bank.className = "focus-button";
    bank.type = "button";
    bank.disabled = !canBank;
    bank.textContent = canBank ? `✓ ${careText("focusBank")}` : careText("focusNeedTime");
    bank.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      finishFocusSession(pet);
    });

    const cancel = document.createElement("button");
    cancel.className = "focus-button is-cancel";
    cancel.type = "button";
    cancel.disabled = !mine;
    cancel.textContent = `× ${careText("focusCancel")}`;
    cancel.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      cancelFocusSession(pet);
    });
    actions.append(bank, cancel);
  } else {
    for (const minutes of FOCUS_PRESETS) {
      const button = document.createElement("button");
      button.className = "focus-button";
      button.type = "button";
      button.textContent = `▶ ${minutes}m`;
      button.title = `${careText("focusReward")} · ${focusRewardText(focusReward(minutes, minutes))}`;
      button.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        startFocusSession(pet, minutes);
      });
      actions.appendChild(button);
    }
  }

  card.append(head, intro, meter, facts, actions);
  return card;
}

function renderNurseryPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const nest = eggNestFor(game);
  const enoughEnergy = care.energy >= 6;
  const card = document.createElement("section");
  card.className = "nursery-card";

  const head = document.createElement("div");
  head.className = "nursery-head";
  const title = document.createElement("strong");
  title.textContent = careText("nurseryTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("nurseryHatched")} ${nest.hatchedCount}`;
  head.append(title, count);

  const main = document.createElement("div");
  main.className = "nursery-main";
  const egg = document.createElement("span");
  egg.className = "nursery-egg";
  egg.textContent = "EG";
  const info = document.createElement("div");
  info.className = "nursery-info";
  const progressLabel = document.createElement("strong");
  progressLabel.textContent = nest.progress >= 100 ? careText("nurseryReady") : `${careText("nurseryProgress")} ${nest.progress}/100`;
  const hint = document.createElement("span");
  hint.textContent = nurseryStatusText(game);
  info.append(progressLabel, hint);
  main.append(egg, info);

  const meter = document.createElement("div");
  meter.className = "nursery-meter";
  const meterTop = document.createElement("div");
  meterTop.className = "care-meter__top";
  const meterLabel = document.createElement("span");
  meterLabel.textContent = careText("nurseryHint");
  const meterValue = document.createElement("strong");
  meterValue.textContent = `${nest.progress}%`;
  meterTop.append(meterLabel, meterValue);
  const bar = document.createElement("i");
  bar.style.setProperty("--value", `${nest.progress}%`);
  meter.append(meterTop, bar);

  const facts = document.createElement("div");
  facts.className = "nursery-facts";
  for (const item of [
    { label: careText("energy"), value: "-6" },
    { label: careText("nurseryReward"), value: `+3 ${careText("coins")} + ${careText("xp")}` },
    { label: careText("nurseryNext"), value: characterFor(nextHatchCharacterId()).name },
  ]) {
    const fact = document.createElement("span");
    const label = document.createElement("em");
    label.textContent = item.label;
  ...
  button.type = "button";
  button.disabled = !enoughEnergy;
  button.textContent = enoughEnergy ? `+ ${careText("nurseryWarm")}` : careText("nurseryNeedEnergy");
  button.title = enoughEnergy ? careText("nurseryHint") : careText("nurseryNeedEnergy");
  button.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    performEggCare(pet);
  ...
  card.append(head, main, meter, facts, button);
  return card;
}

function renderSnackPanel(pet) {
  const game = ensureGame();
  const care = careForPet(pet);
  const favorites = favoriteSnacksForPet(pet);
  const card = document.createElement("section");
  ...
  const head = document.createElement("div");
  head.className = "snack-head";
  const title = document.createElement("strong");
  title.textContent = careText("snackPantryTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("snackServed")} ${snackUseCount(care)}`;
  head.append(title, count);
  ...
  for (const snackId of SNACK_IDS) {
    const snack = SNACKS[snackId];
    const owned = snackOwnedCount(game, snackId);
    const favorite = favorites.includes(snackId);
    const reward = snackReward(snack, favorite);
    const canBuy = owned > 0 || game.coins >= snack.cost;
    const button = document.createElement("button");
    button.className = "snack-button";
    button.type = "button";
    button.classList.toggle("is-favorite", favorite);
    button.classList.toggle("is-empty", owned <= 0);
    button.classList.toggle("is-locked", !canBuy);
    button.title = `${careText(snack.labelKey)} · ${careText(snack.descKey)}`;
    button.style.setProperty("--snack-color", snack.color || "#facc15");
    ...
    const info = document.createElement("span");
    info.className = "snack-info";
    const name = document.createElement("strong");
    name.textContent = careText(snack.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(snack.descKey);
    const meta = document.createElement("em");
    meta.textContent = favorite
      ? `${careText("snackFavorite")} · ${careText("snackOwned")} ${owned}`
      : `${careText("snackOwned")} ${owned} · ${careText("snackBuy")} ${snack.cost}`;
    const rewardLine = document.createElement("small");
    rewardLine.textContent = snackRewardText(reward);
    info.append(name, desc, meta, rewardLine);

    const action = document.createElement("span");
    action.className = "snack-action";
    action.textContent = owned > 0 ? `+ ${careText("snackServe")}` : `${careText("snackBuy")} ${snack.cost}`;

    button.append(icon, info, action);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      buyOrServeSnack(pet, snackId);
    });
    grid.appendChild(button);
  }
...
function renderSignaturePanel(pet) {
  const care = careForPet(pet);
  const action = signatureActionFor(pet);
  const count = Math.round(clamp(Number(care.actionCounts?.[signatureActionKeyFor(pet)]) || 0, 0, 999999));
  const enoughEnergy = care.energy >= action.energyCost;
  const card = document.createElement("section");
  ...
  const title = document.createElement("strong");
  title.textContent = careText("specialTitle");
  const badge = document.createElement("span");
  badge.textContent = `${careText("specialTimes")} ${count}`;
  head.append(title, badge);
  ...
  icon.textContent = action.icon;
  icon.style.setProperty("--signature-color", action.color || "#42d7c5");
  const info = document.createElement("div");
  info.className = "signature-info";
  const name = document.createElement("strong");
  name.textContent = signatureName(action);
  const desc = document.createElement("span");
  desc.textContent = signatureDesc(action);
  info.append(name, desc);
  main.append(icon, info);
  ...
  for (const item of [
    { label: careText("energy"), value: `-${action.energyCost}` },
    { label: careText("specialReward"), value: signatureRewardText(action) },
    { label: careText("specialReady"), value: enoughEnergy ? careText("done") : careText("specialNeedEnergy") },
  ]) {
    const fact = document.createElement("span");
    const label = document.createElement("em");
    label.textContent = item.label;
  ...
  button.classList.toggle("is-tired", !enoughEnergy);
  button.type = "button";
  button.textContent = enoughEnergy ? `+ ${careText("specialUse")}` : careText("specialNeedEnergy");
  button.title = `${signatureName(action)} · ${signatureRewardText(action)}`;
  button.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    performSignatureAction(pet);
  });

  card.append(head, main, facts, button);
  return card;
}
...
function renderQuestPanel() {
  ...
  const title = document.createElement("strong");
  title.textContent = careText("questsTitle");
  const coins = document.createElement("span");
  coins.textContent = `${careText("coins")} ${game.coins}`;
  ...
  for (const quest of game.dailyQuests) {
    ...
    const label = document.createElement("strong");
    label.textContent = careText(quest.labelKey);
    const meta = document.createElement("span");
    meta.textContent = quest.done
      ? `${careText("done")} · +${quest.reward} ${careText("coins")}`
      : `${quest.progress}/${quest.target} · ${careText("questReward")} +${quest.reward}`;
    text.append(label, meta);
    const bar = document.createElement("i");
    bar.style.setProperty("--value", `${clamp((quest.progress / quest.target) * 100, 0, 100)}%`);
    row.append(text, bar);
...
function renderMilestonePanel(pet) {
  const game = ensureGame();
  const care = careForPet(pet);
  const claimed = claimedMilestoneSet(game);
  const card = document.createElement("section");
  ...
  const title = document.createElement("strong");
  title.textContent = careText("milestonesTitle");
  const count = document.createElement("span");
  count.textContent = `${claimed.size}/${MILESTONE_IDS.length}`;
  head.append(title, count);
  ...
  for (const milestone of MILESTONE_DEFS) {
    ...
    const name = document.createElement("strong");
    name.textContent = careText(milestone.labelKey);
    const desc = document.createElement("span");
    desc.className = "milestone-desc";
    desc.textContent = careText(milestone.descKey);
    const meta = document.createElement("span");
    meta.className = "milestone-meta";
    meta.textContent = `${Math.min(progress.value, progress.target)}/${progress.target} · ${careText("milestoneReward")} ${milestoneRewardText(milestone)}`;
    const bar = document.createElement("i");
    bar.className = "milestone-bar";
    bar.style.setProperty("--value", `${progress.percent}%`);
    info.append(name, desc, meta, bar);
    ...
    button.disabled = !isReady;
    button.textContent = isClaimed
      ? careText("milestoneClaimed")
      : isReady
        ? careText("milestoneClaim")
        : careText("locked");
    button.title = isReady ? `${careText("milestoneClaim")} · ${milestoneRewardText(milestone)}` : careText(milestone.descKey);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      claimMilestone(pet, milestone.id);
    });
...
function renderHabitatPanel(pet) {
  const game = ensureGame();
  const card = document.createElement("section");
  card.className = "habitat-card";

  const head = document.createElement("div");
  head.className = "habitat-head";
  const title = document.createElement("strong");
  title.textContent = careText("habitatTitle");
  const comfort = document.createElement("span");
  comfort.textContent = `${careText("habitatComfort")} ${habitatComfortScore(game)}`;
  head.append(title, comfort);

  const themeSection = document.createElement("div");
  themeSection.className = "habitat-themes";
  const themeHead = document.createElement("div");
  themeHead.className = "habitat-subhead";
  const themeTitle = document.createElement("strong");
  themeTitle.textContent = careText("habitatThemesTitle");
  const activeTheme = habitatThemeFor(game);
  const themeMeta = document.createElement("span");
  themeMeta.textContent = `${careText("habitatTheme")} · ${careText(activeTheme.labelKey)}`;
  themeHead.append(themeTitle, themeMeta);
  const themeGrid = document.createElement("div");
  themeGrid.className = "habitat-theme-grid";
  for (const themeId of HABITAT_THEME_IDS) {
    const theme = HABITAT_THEMES[themeId];
    const active = game.habitatTheme === themeId;
    const unlocked = habitatThemeUnlocked(game, theme);
    const button = document.createElement("button");
    button.className = "habitat-theme-button";
    button.type = "button";
    button.classList.toggle("is-active", active);
    button.classList.toggle("is-locked", !unlocked);
    button.style.setProperty("--theme-color", theme.color);
    button.title = `${careText(theme.labelKey)} · ${careText(theme.descKey)}`;
    const icon = document.createElement("span");
    icon.textContent = theme.icon;
    const info = document.createElement("span");
    const name = document.createElement("strong");
    name.textContent = careText(theme.labelKey);
    const meta = document.createElement("em");
    meta.textContent = active
      ? careText("habitatThemeActive")
      : unlocked
        ? careText("habitatThemeUse")
        : `${careText("habitatThemeLocked")} ${theme.unlockComfort}`;
    info.append(name, meta);
    button.append(icon, info);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      switchHabitatTheme(pet, themeId);
    });
    themeGrid.appendChild(button);
  }
  themeSection.append(themeHead, themeGrid);

  const room = document.createElement("div");
  room.className = "habitat-room";
  for (let index = 0; index < HABITAT_SLOT_LIMIT; index += 1) {
    const itemId = game.habitatLayout[index] || "";
    const item = HABITAT_ITEMS[itemId];
    ...
    if (item) {
      slot.type = "button";
      slot.title = `${careText("habitatRemove")} ${careText(item.labelKey)}`;
      const icon = document.createElement("span");
      icon.textContent = item.icon;
      const name = document.createElement("strong");
      name.textContent = careText(item.labelKey);
      slot.append(icon, name);
      slot.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        buyOrToggleHabitatItem(pet, itemId);
      });
    } else {
      const icon = document.createElement("span");
      icon.textContent = "+";
      const name = document.createElement("strong");
      name.textContent = careText("habitatEmptySlot");
      slot.append(icon, name);
    }
  ...
  bonusSection.className = "habitat-bonus";
  const bonusHead = document.createElement("div");
  bonusHead.className = "habitat-subhead";
  const bonusTitle = document.createElement("strong");
  bonusTitle.textContent = careText("habitatSetBonus");
  const activeBonuses = habitatSetBonuses(game);
  const bonusMeta = document.createElement("span");
  bonusMeta.textContent = `${activeBonuses.length}/${HABITAT_SET_BONUS_IDS.length}`;
  bonusHead.append(bonusTitle, bonusMeta);
  const bonusList = document.createElement("div");
  bonusList.className = "habitat-bonus-list";
  for (const bonusId of HABITAT_SET_BONUS_IDS) {
    const bonus = HABITAT_SET_BONUSES[bonusId];
    const active = activeBonuses.some((item) => item.id === bonusId);
    const row = document.createElement("div");
    row.className = "habitat-bonus-item";
    row.classList.toggle("is-active", active);
    const icon = document.createElement("span");
    icon.textContent = bonus.icon;
    const info = document.createElement("span");
    const name = document.createElement("strong");
    name.textContent = careText(bonus.labelKey);
    const desc = document.createElement("em");
    desc.textContent = active ? roomRewardText(bonus.reward) : careText(bonus.descKey);
    info.append(name, desc);
    row.append(icon, info);
    bonusList.appendChild(row);
  }
  bonusSection.append(bonusHead, bonusList);

  const restButton = document.createElement("button");
  restButton.className = "habitat-rest";
  restButton.type = "button";
  restButton.textContent = `▣ ${careText("habitatRest")}`;
  ...
  playButton.className = "habitat-play";
  playButton.type = "button";
  playButton.disabled = habitatPlacedCount(game) <= 0;
  playButton.textContent = `+ ${careText("habitatRoomPlay")}`;
  playButton.title = `${careText("habitatRoomReward")} · ${roomRewardText(habitatRoomReward(game))}`;
  ...
  const actions = document.createElement("div");
  actions.className = "habitat-actions";
  actions.append(restButton, playButton);

  const shop = document.createElement("div");
  shop.className = "habitat-grid";
  for (const itemId of HABITAT_IDS) {
    const item = HABITAT_ITEMS[itemId];
    const owned = game.habitatInventory.includes(itemId);
    const placed = game.habitatLayout.includes(itemId);
    const full = game.habitatLayout.length >= HABITAT_SLOT_LIMIT;
    const button = document.createElement("button");
    button.className = "habitat-button";
    button.type = "button";
    button.classList.toggle("is-owned", owned);
    button.classList.toggle("is-placed", placed);
    button.disabled = owned && !placed && full;
    button.title = `${careText(item.labelKey)} · ${careText(item.descKey)}`;
    ...
    meta.textContent = placed
      ? careText("habitatRemove")
      : owned
        ? full
          ? careText("habitatFull")
          : careText("habitatPlace")
        : `${careText("habitatBuy")} · ${habitatItemRewardText(item)}`;
    const desc = document.createElement("em");
    desc.textContent = careText(item.descKey);
  ...
  card.append(head, themeSection, room, bonusSection, actions, shop);
  return card;
}

function renderRoomEventPanel(pet) {
  const game = ensureGame();
  const care = careForPet(pet);
  const card = document.createElement("section");
  card.className = "room-event-card";

  const head = document.createElement("div");
  head.className = "room-event-head";
  const title = document.createElement("strong");
  title.textContent = careText("roomEventTitle");
  const total = document.createElement("span");
  total.textContent = `${careText("roomEventDone")} ${totalRoomEvents(care)}`;
  head.append(title, total);

  const grid = document.createElement("div");
  grid.className = "room-event-grid";
  for (const eventId of ROOM_EVENT_IDS) {
    const event = ROOM_EVENTS[eventId];
    const unlocked = roomEventUnlocked(game, event);
    const ready = roomEventReady(game, care, event);
    const count = roomEventCount(care, eventId);
    const button = document.createElement("button");
    button.className = "room-event-button";
    button.type = "button";
    button.classList.toggle("is-locked", !ready);
    button.style.setProperty("--room-event-color", event.color);
    button.title = unlocked
      ? `${careText("roomEventRun")} ${careText(event.labelKey)} · -${event.energyCost} ${careText("energy")}`
      : roomEventRequirementText(event);

    const icon = document.createElement("span");
    icon.className = "room-event-icon";
    icon.textContent = event.icon;

    const info = document.createElement("span");
    info.className = "room-event-info";
    const name = document.createElement("strong");
    name.textContent = careText(event.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(event.descKey);
    const meta = document.createElement("em");
    meta.textContent = unlocked
      ? `${careText("done")} ${count} · -${event.energyCost} ${careText("energy")}`
      : roomEventItemsText(event);
    info.append(name, desc, meta);

    const action = document.createElement("span");
    action.className = "room-event-action";
    action.textContent = ready
      ? `+ ${careText("roomEventRun")}`
      : careText(unlocked ? "roomEventNeedEnergy" : "roomEventNeedItems");

    button.append(icon, info, action);
    button.addEventListener("click", (eventObject) => {
      eventObject.preventDefault();
      eventObject.stopPropagation();
      performRoomEvent(pet, eventId);
    });
    grid.appendChild(button);
  }
...
function renderToyPanel(pet) {
  ...
  const head = document.createElement("div");
  head.className = "toy-head";
  const title = document.createElement("strong");
  title.textContent = careText("toyBox");
  const coins = document.createElement("span");
  coins.textContent = `${careText("coins")} ${game.coins}`;
  head.append(title, coins);

  const grid = document.createElement("div");
  grid.className = "toy-grid";
  for (const toyId of TOY_IDS) {
    const toy = TOYS[toyId];
    const owned = game.inventory.includes(toyId);
    const equipped = care.equippedToy === toyId;
    const canBuy = owned || game.coins >= toy.cost;
    const button = document.createElement("button");
    ...
    button.classList.toggle("is-locked", !canBuy);
    button.title = `${careText(toy.labelKey)} · ${owned ? careText("owned") : `${toy.cost} ${careText("coins")}`}`;

    const icon = document.createElement("span");
    icon.className = "toy-icon";
    icon.textContent = toy.icon;

    const info = document.createElement("span");
    info.className = "toy-info";
    const name = document.createElement("strong");
    name.textContent = careText(toy.labelKey);
    const meta = document.createElement("span");
    meta.textContent = equipped
      ? careText("equipped")
      : owned
        ? careText("equip")
        : `${careText("buy")} · ${toy.cost}`;
    info.append(name, meta);
...
function renderEffectPanel(pet) {
  const game = ensureGame();
  const care = careForPet(pet);
  const inventory = normalizeEffectInventory(game.effectInventory);
  const equipped = effectForCare(care, pet);
  const card = document.createElement("section");
  ...
  const title = document.createElement("strong");
  title.textContent = careText("effectShopTitle");
  const coins = document.createElement("span");
  coins.textContent = `${careText("coins")} ${game.coins}`;
  head.append(title, coins);
  ...
  for (const effectId of EFFECT_IDS) {
    const effect = TRAIL_STYLES[effectId];
    const owned = inventory.includes(effectId);
    const isEquipped = equipped.mode === effect.mode;
    const canBuy = owned || game.coins >= effect.cost;
    const button = document.createElement("button");
    ...
    button.classList.toggle("is-locked", !canBuy);
    button.title = `${careText(effect.labelKey)} · ${careText(effect.descKey)}`;
    ...
    const name = document.createElement("strong");
    name.textContent = careText(effect.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(effect.descKey);
    const meta = document.createElement("em");
    meta.textContent = isEquipped
      ? careText("effectEquipped")
      : owned
        ? careText("effectEquip")
        : `${careText("effectBuy")} · ${effect.cost}`;
    info.append(name, desc, meta);

    const reward = document.createElement("span");
    reward.className = "effect-reward";
    reward.textContent = effectRewardText(effect);

    button.append(icon, info, reward);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      buyOrEquipEffect(pet, effectId);
    });
    grid.appendChild(button);
  }
...
function renderToyPlayPanel(pet) {
  const care = careForPet(pet);
  const toyId = care.equippedToy;
  const toy = TOYS[toyId] || null;
  const masteryLevel = toy ? toyPlayMasteryLevel(care, toyId) : 0;
  const count = toy ? toyPlayCount(care, toyId) : 0;
  const target = toy ? toyPlayNextTarget(care, toyId) : TOY_PLAY_MASTERY_THRESHOLDS[0];
  const reward = toy ? toyPlayReward(toy, masteryLevel) : null;
  const card = document.createElement("section");
  card.className = "toy-play-card";
  card.classList.toggle("is-empty", !toy);

  const head = document.createElement("div");
  head.className = "toy-play-head";
  const title = document.createElement("strong");
  title.textContent = careText("toyPlayTitle");
  const badge = document.createElement("span");
  badge.textContent = toy ? `${careText("toyPlayMastery")} ${masteryLevel}/${TOY_PLAY_MASTERY_THRESHOLDS.length}` : careText("toyPlayNeed");
  head.append(title, badge);

  const body = document.createElement("div");
  body.className = "toy-play-body";
  const icon = document.createElement("span");
  icon.className = "toy-play-icon";
  icon.textContent = toy?.icon || "◆";
  const info = document.createElement("div");
  info.className = "toy-play-info";
  const name = document.createElement("strong");
  name.textContent = toy ? careText(toy.labelKey) : careText("toyPlayNeed");
  const meta = document.createElement("span");
  meta.textContent = toy
    ? `${careText("toyPlayNext")} ${Math.min(count, target)}/${target} · ${careText("toyPlayReward")} ${toyPlayRewardText(reward)}`
    : careText("toyPlayNeed");
  const bar = document.createElement("i");
  bar.style.setProperty("--value", toy ? `${clamp((count / target) * 100, 0, 100)}%` : "0%");
  info.append(name, meta, bar);
  body.append(icon, info);

  const button = document.createElement("button");
  button.className = "toy-play-button";
  button.type = "button";
  button.disabled = !toy;
  button.textContent = toy ? `+ ${careText("toyPlay")}` : careText("toyPlayNeed");
  button.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    performToyPlay(pet);
  ...
  card.append(head, body, button);
  return card;
}

function renderDesktopObjectPanel(pet) {
  const care = careForPet(pet);
  const card = document.createElement("section");
  card.className = "desk-object-card";

  const head = document.createElement("div");
  head.className = "desk-object-head";
  const title = document.createElement("strong");
  title.textContent = careText("deskObjectTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("deskObjectCount")} ${totalDesktopObjects(care)}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "desk-object-grid";
  for (const objectId of DESKTOP_OBJECT_IDS) {
    const object = DESKTOP_OBJECTS[objectId];
    const canUse = care.energy >= object.energyCost + 2;
    const button = document.createElement("button");
    button.className = "desk-object-button";
    button.classList.toggle("is-locked", !canUse);
    button.type = "button";
    button.title = `${careText(object.descKey)} · ${desktopObjectRewardText(object.reward)}`;
    button.style.setProperty("--desk-object-color", object.color);

    const icon = document.createElement("span");
    icon.className = "desk-object-icon";
    icon.textContent = object.icon;

    const info = document.createElement("span");
    info.className = "desk-object-info";
    const name = document.createElement("strong");
    name.textContent = careText(object.labelKey);
    const meta = document.createElement("span");
    meta.textContent = `${careText("deskObjectCount")} ${desktopObjectCount(care, objectId)} · -${object.energyCost} ${careText("energy")}`;
    const desc = document.createElement("em");
    desc.textContent = careText(object.descKey);
    info.append(name, meta, desc);

    const action = document.createElement("span");
    action.className = "desk-object-action";
    action.textContent = canUse ? `+ ${careText("deskObjectThrow")}` : careText("deskObjectNeedEnergy");

    button.append(icon, info, action);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      performDesktopObject(pet, objectId);
    });
    grid.appendChild(button);
  }
...
function renderMiniGamePanel(pet) {
  const care = careForPet(pet);
  const card = document.createElement("section");
  card.className = "mini-game-card";

  const head = document.createElement("div");
  head.className = "mini-game-head";
  const title = document.createElement("strong");
  title.textContent = careText("miniGameTitle");
  const total = document.createElement("span");
  total.textContent = `${careText("done")} ${totalMiniGames(care)}`;
  head.append(title, total);
  ...
  for (const gameId of MINI_GAME_IDS) {
    const gameDef = MINI_GAMES[gameId];
    const best = miniGameBest(care, gameId);
    const count = miniGameCount(care, gameId);
    const canPlay = care.energy >= gameDef.energyCost + 4;
    const button = document.createElement("button");
    button.className = "mini-game-button";
    button.type = "button";
    button.classList.toggle("is-locked", !canPlay);
    button.title = `${careText(gameDef.labelKey)} · ${careText(gameDef.descKey)}`;

    const icon = document.createElement("span");
    icon.className = "mini-game-icon";
    icon.textContent = gameDef.icon;

    const info = document.createElement("span");
    info.className = "mini-game-info";
    const name = document.createElement("strong");
    name.textContent = careText(gameDef.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(gameDef.descKey);
    const meta = document.createElement("em");
    meta.textContent = `${careText("miniGameBest")} ${best} · ${careText("done")} ${count} · -${gameDef.energyCost} ${careText("energy")}`;
    info.append(name, desc, meta);

    const action = document.createElement("span");
    action.className = "mini-game-action";
    action.textContent = canPlay ? `+ ${careText("miniGamePlay")}` : careText("miniGameNeed");

    button.append(icon, info, action);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      performMiniGame(pet, gameId);
    });
    grid.appendChild(button);
  }
...
function renderTalentPanel(pet) {
  const care = careForPet(pet);
  const card = document.createElement("section");
  ...
  const title = document.createElement("strong");
  title.textContent = careText("talentSchoolTitle");
  const total = document.createElement("span");
  total.textContent = `${careText("talentLevel")} ${totalTalentLevels(care)}/${TALENT_IDS.length * TALENT_MAX_LEVEL}`;
  head.append(title, total);
  ...
  for (const talentId of TALENT_IDS) {
    const talent = TALENTS[talentId];
    const count = talentTrainingCount(care, talentId);
    const level = talentLevel(care, talentId);
    const next = talentNextTarget(care, talentId);
    const progress = level >= TALENT_MAX_LEVEL ? 100 : clamp((count / next) * 100, 0, 100);
    const ready = talentReady(care, talent);
    const button = document.createElement("button");
    button.className = "talent-button";
    button.type = "button";
    button.classList.toggle("is-locked", !ready);
    button.style.setProperty("--talent-color", talent.color);
    button.title = ready
      ? `${careText("talentTrain")} ${careText(talent.labelKey)} · ${talentRequirementText(talent)}`
      : `${careText("talentNeedEnergy")} / ${careText("talentNeedSkill")} · ${talentRequirementText(talent)}`;
    ...
    const name = document.createElement("strong");
    name.textContent = careText(talent.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(talent.descKey);
    const meta = document.createElement("em");
    meta.textContent = `${careText("talentLevel")} ${level}/${TALENT_MAX_LEVEL} · ${careText("talentProgress")} ${count}/${next}`;
    const bar = document.createElement("i");
    ...
    action.textContent = ready ? `+ ${careText("talentTrain")}` : careText(care.energy < talent.energyCost + 4 ? "talentNeedEnergy" : "talentNeedSkill");

    button.append(icon, info, action);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      performTalentTraining(pet, talentId);
    });
    grid.appendChild(button);
  }
...
function renderTinyJobPanel(pet) {
  const care = careForPet(pet);
  const card = document.createElement("section");
  ...
  const title = document.createElement("strong");
  title.textContent = careText("jobBoardTitle");
  const total = document.createElement("span");
  total.textContent = `${careText("done")} ${totalTinyJobs(care)}`;
  head.append(title, total);

  const grid = document.createElement("div");
  grid.className = "job-grid";
  for (const jobId of TINY_JOB_IDS) {
    const job = TINY_JOBS[jobId];
    const count = tinyJobRunCount(care, jobId);
    const reputation = tinyJobReputation(care, jobId);
    const next = tinyJobNextTarget(care, jobId);
    const progress = reputation >= JOB_REPUTATION_THRESHOLDS.length ? 100 : clamp((count / next) * 100, 0, 100);
    const ready = tinyJobReady(care, job);
    const button = document.createElement("button");
    button.className = "job-button";
    button.type = "button";
    button.classList.toggle("is-locked", !ready);
    button.style.setProperty("--job-color", job.color);
    button.title = ready
      ? `${careText("jobRun")} ${careText(job.labelKey)} · ${tinyJobRequirementText(job)}`
      : tinyJobRequirementText(job);
    ...
    const name = document.createElement("strong");
    name.textContent = careText(job.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(job.descKey);
    const meta = document.createElement("em");
    meta.textContent = `${careText("jobReputation")} ${reputation}/${JOB_REPUTATION_THRESHOLDS.length} · ${careText("talentProgress")} ${count}/${next}`;
    const bar = document.createElement("i");
    bar.style.setProperty("--value", `${progress}%`);
    info.append(name, desc, meta, bar);
    ...
    const lockedKey = care.level < job.level ? "jobNeedLevel" : care.training < job.training ? "jobNeedSkill" : "jobNeedEnergy";
    action.textContent = ready ? `+ ${careText("jobRun")}` : careText(lockedKey);

    button.append(icon, info, action);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      performTinyJob(pet, jobId);
    });
    grid.appendChild(button);
  }
...
function renderTrickPanel(pet) {
  const care = careForPet(pet);
  const card = document.createElement("section");
  card.className = "trick-card";

  const head = document.createElement("div");
  head.className = "trick-head";
  const title = document.createElement("strong");
  title.textContent = careText("tricksTitle");
  const count = document.createElement("span");
  const unlockedCount = PET_TRICK_IDS.filter((id) => trickUnlocked(care, PET_TRICKS[id])).length;
  count.textContent = `${unlockedCount}/${PET_TRICK_IDS.length}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "trick-grid";
  for (const trickId of PET_TRICK_IDS) {
    const trick = PET_TRICKS[trickId];
    const unlocked = trickUnlocked(care, trick);
    const button = document.createElement("button");
    button.className = "trick-button";
    button.classList.toggle("is-locked", !unlocked);
    button.type = "button";
    const masteryLevel = trickMasteryLevel(care, trickId);
    button.title = unlocked
      ? `${careText("trickUse")} ${careText(trick.labelKey)} · ${careText("trickMastery")} ${masteryLevel}/${TRICK_MASTERY_THRESHOLDS.length}`
      : trickRequirementText(trick);

    const icon = document.createElement("span");
    icon.className = "trick-icon";
    icon.textContent = trick.icon;

    const info = document.createElement("span");
    info.className = "trick-info";
    const name = document.createElement("strong");
    name.textContent = careText(trick.labelKey);
    const meta = document.createElement("span");
    meta.textContent = unlocked
      ? `${careText("trickMastery")} ${masteryLevel}/${TRICK_MASTERY_THRESHOLDS.length}`
      : trickRequirementText(trick);
    info.append(name, meta);
...
function renderMasteryPanel(pet) {
  const care = careForPet(pet);
  const maxMastery = PET_TRICK_IDS.length * TRICK_MASTERY_THRESHOLDS.length;
  const totalMastery = totalTrickMastery(care);
  const card = document.createElement("section");
  card.className = "mastery-card";

  const head = document.createElement("div");
  head.className = "mastery-head";
  const title = document.createElement("strong");
  title.textContent = careText("trickMasteryTitle");
  const count = document.createElement("span");
  count.textContent = `${totalMastery}/${maxMastery}`;
  head.append(title, count);
  ...
  for (const trickId of PET_TRICK_IDS) {
    const trick = PET_TRICKS[trickId];
    const unlocked = trickUnlocked(care, trick);
    const uses = trickUseCount(care, trickId);
    const level = trickMasteryLevel(care, trickId);
    const target = trickMasteryTarget(care, trickId);
    const complete = level >= TRICK_MASTERY_THRESHOLDS.length;
    const item = document.createElement("article");
    ...
    meta.textContent = complete
      ? `${careText("trickMasteryComplete")} · ${trickMasteryLabel(care, trickId)}`
      : unlocked
        ? `${careText("trickMasteryNext")} ${uses}/${target} · ${trickMasteryLabel(care, trickId)}`
        : trickRequirementText(trick);
    const bar = document.createElement("i");
    const percent = complete ? 100 : clamp((uses / target) * 100, 0, 100);
...
function renderExpeditionPanel(pet) {
  const care = careForPet(pet);
  const card = document.createElement("section");
  ...
  const title = document.createElement("strong");
  title.textContent = careText("expeditionsTitle");
  const count = document.createElement("span");
  const unlockedCount = EXPEDITION_IDS.filter((id) => expeditionUnlocked(care, EXPEDITIONS[id])).length;
  count.textContent = `${unlockedCount}/${EXPEDITION_IDS.length}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "expedition-grid";
  for (const expeditionId of EXPEDITION_IDS) {
    const expedition = EXPEDITIONS[expeditionId];
    const unlocked = expeditionUnlocked(care, expedition);
    const enoughEnergy = care.energy >= expedition.energyCost;
    const button = document.createElement("button");
    button.className = "expedition-button";
    button.classList.toggle("is-locked", !unlocked);
    button.classList.toggle("is-tired", unlocked && !enoughEnergy);
    button.type = "button";
    button.title = unlocked
      ? `${careText("expeditionGo")} · -${expedition.energyCost} ${careText("energy")}`
      : expeditionRequirementText(expedition);

    const icon = document.createElement("span");
    icon.className = "expedition-icon";
    icon.textContent = expedition.icon;

    const info = document.createElement("span");
    info.className = "expedition-info";
    const name = document.createElement("strong");
    name.textContent = careText(expedition.labelKey);
    const meta = document.createElement("span");
    meta.textContent = unlocked
      ? `${careText("expeditionGo")} · -${expedition.energyCost} ${careText("energy")} · +${expedition.coins} ${careText("coins")}`
      : expeditionRequirementText(expedition);
    info.append(name, meta);
    ...
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      performExpedition(pet, expeditionId);
    });
    grid.appendChild(button);
  }
...
function badgeItems(care, game, pet) {
  return [
    { key: "badgeFirstCare", icon: "✦", unlocked: totalCareActions(care) > 0 },
    { key: "badgeLevel5", icon: "LV", unlocked: care.level >= 5 },
    { key: "badgeBond50", icon: "♥", unlocked: care.bond >= 50 },
    { key: "badgeTraining60", icon: "★", unlocked: care.training >= 60 },
    { key: "badgeDailyDone", icon: "✓", unlocked: allDailyQuestsDone(game) },
    { key: "badgeToyOwner", icon: "◆", unlocked: game.inventory.length > 0 },
    { key: "badgeToyBuddy", icon: "TY", unlocked: totalToyPlay(care) >= 10 },
    { key: "badgeWalkBuddy", icon: "WL", unlocked: totalPetWalks(care) >= 10 },
    { key: "badgeYardTrainer", icon: "YD", unlocked: totalTrainingYardRuns(care) >= 12 },
    { key: "badgePatrolScout", icon: "PT", unlocked: totalPatrolRuns(care) >= 18 },
    { key: "badgeMiniGamer", icon: "MG", unlocked: totalMiniGames(care) >= 10 },
    { key: "badgeTrailStylist", icon: "FX", unlocked: effectOwnedCount(game) >= 4 },
    { key: "badgeNestKeeper", icon: "EG", unlocked: eggHatchCount(game) >= 3 },
    { key: "badgeSnackChef", icon: "SN", unlocked: totalFavoriteSnacks(care) >= 10 },
    { key: "badgeMemoryKeeper", icon: "M", unlocked: (care.memories || []).length >= 6 },
    { key: "badgeBestFriend", icon: "BF", unlocked: Object.values(care.friendships || {}).some((score) => score >= 120) },
    { key: "badgeCollector", icon: "C", unlocked: collectionUniqueCount(game) >= Math.ceil(DISCOVERY_ITEM_IDS.length / 2) },
    { key: "badgeCharmCrafter", icon: "CH", unlocked: totalCharmCrafts(care) >= CHARM_IDS.length },
    { key: "badgePetAlbum", icon: "PA", unlocked: petAlbumSeenCount(game) >= 10 },
    { key: "badgeMoodReader", icon: "MR", unlocked: totalMoodChecks(care) >= 12 },
    { key: "badgeMoodPattern", icon: "MP", unlocked: totalMoodPatterns(care) >= 3 },
    { key: "badgeTalentGraduate", icon: "TG", unlocked: totalTalentLevels(care) >= 9 },
    { key: "badgeJobHelper", icon: "JW", unlocked: totalTinyJobs(care) >= 12 },
    { key: "badgeFocusBuddy", icon: "FO", unlocked: totalFocusSessions(care) >= 8 },
    { key: "badgeMomentMaker", icon: "MO", unlocked: totalMicroEvents(care) >= 20 },
    { key: "badgeGrowthPath", icon: "GP", unlocked: growthPathFor(care, game).score >= 150 },
    { key: "badgeGrowthReward", icon: "GR", unlocked: totalGrowthRewards(care) >= GROWTH_REWARD_IDS.length },
    { key: "badgeQuirkKeeper", icon: "QK", unlocked: totalCareQuirks(care) >= 5 },
    { key: "badgeQuirkCombo", icon: "QC", unlocked: totalCareQuirkCombos(care) >= 3 },
    { key: "badgeHabitKeeper", icon: "HB", unlocked: totalPetHabits(care) >= PET_HABIT_IDS.length },
    { key: "badgeInstinctKeeper", icon: "IN", unlocked: totalAnimalInstincts(pet, care, game) > 0 },
    { key: "badgePerkKeeper", icon: "BP", unlocked: totalBondPerks(care, game) >= BOND_PERK_IDS.length },
    { key: "badgeCaretakerRank", icon: "CR", unlocked: caretakerRankFor(game).index >= 2 },
    { key: "badgeSynergyKeeper", icon: "SY", unlocked: totalPetSynergies(pet, care, game) >= PET_SYNERGY_IDS.length },
    { key: "badgeStoryArchivist", icon: "ST", unlocked: totalLifeStoryChapters(pet, care, game) >= LIFE_STORY_CHAPTER_IDS.length },
    { key: "badgePlaydateHost", icon: "PD", unlocked: totalPlaydates(care) >= 6 },
    { key: "badgeDuoBuddy", icon: "DU", unlocked: totalDuoMoves(care) >= 10 },
    { key: "badgePackLeader", icon: "PK", unlocked: totalPackEvents(care) >= 8 },
    { key: "badgeContestChamp", icon: "LC", unlocked: totalContestRuns(care) >= 10 || PET_CONTEST_IDS.some((id) => contestBestScore(care, id) >= 180) },
    { key: "badgeLeagueCollector", icon: "LS", unlocked: claimedLeagueSeasonTierCount(game) >= LEAGUE_SEASON_TIERS.length },
    { key: "badgeMedalCollector", icon: "MD", unlocked: unlockedMedalCount(care, game) >= LEAGUE_MEDAL_IDS.length },
    { key: "badgeMedalRunner", icon: "MC", unlocked: totalMedalTrials(care) >= 12 },
    { key: "badgeObjectPal", icon: "OB", unlocked: totalDesktopObjects(care) >= 12 },
    { key: "badgeRoutineKeeper", icon: "RT", unlocked: totalCareRoutines(care) >= 10 },
    { key: "badgeCommandTrainer", icon: "CM", unlocked: totalPetCommands(care) >= 16 },
    { key: "badgeEvolutionKeeper", icon: "FM", unlocked: totalEvolutionForms(care) >= EVOLUTION_FORM_IDS.length },
    { key: "badgeTrickStar", icon: "TS", unlocked: totalTrickUses(care) >= 10 },
    { key: "badgeMasterTrainer", icon: "TM", unlocked: masteredTrickCount(care) >= 2 || totalTrickMastery(care) >= 16 },
    { key: "badgeExplorer", icon: "EX", unlocked: totalExpeditionRuns(care) >= 6 },
    { key: "badgeMilestoneKeeper", icon: "GK", unlocked: claimedMilestoneSet(game).size >= 4 },
    { key: "badgeRoomMaker", icon: "HM", unlocked: habitatPlacedCount(game) >= 4 },
    { key: "badgeRoomDesigner", icon: "RM", unlocked: totalRoomPlay(care) >= 8 },
    { key: "badgeRoomEventHost", icon: "RE", unlocked: totalRoomEvents(care) >= 14 },
    { key: "badgePersonalityMatch", icon: "PM", unlocked: personalityLikedActionCount(pet, care) >= 8 },
    { key: "badgeSeasonPal", icon: "VB", unlocked: totalAmbientEventCount(game) >= 12 },
    { key: "badgeDailyStreak", icon: "ST", unlocked: dailyStreakValue(game) >= 5 },
    { key: "badgeSignatureStar", icon: "SG", unlocked: totalSignatureActions(care) >= 12 },
    { key: "badgeRequestHelper", icon: "RQ", unlocked: totalCareRequests(care) >= 10 },
    { key: "badgeComboPal", icon: "CB", unlocked: totalCareCombos(care) >= 8 },
  ];
}

function renderBadgePanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const card = document.createElement("section");
  ...
  title.textContent = careText("badgesTitle");

  const grid = document.createElement("div");
  grid.className = "badge-grid";
  for (const badge of badgeItems(care, game, pet)) {
    const item = document.createElement("div");
    item.className = "badge-item";
    item.classList.toggle("is-locked", !badge.unlocked);
    const icon = document.createElement("span");
    ...
    const name = document.createElement("strong");
    name.textContent = careText(badge.key);
    const state = document.createElement("span");
    state.textContent = badge.unlocked ? careText("done") : careText("locked");
...
function renderFriendsPanel(pet) {
  const card = document.createElement("section");
  card.className = "friend-card";

  const title = document.createElement("strong");
  title.className = "friend-title";
  title.textContent = careText("friendsTitle");
  ...
  if (!friends.length) {
    ...
    empty.textContent = careText("noFriends");
    ...
    for (const friend of friends) {
      ...
      icon.textContent = friend.character.name.slice(0, 2).toUpperCase();
      const info = document.createElement("span");
      info.className = "friend-info";
      const name = document.createElement("strong");
      name.textContent = friend.character.name;
      const meta = document.createElement("span");
      meta.textContent = `${friendshipLabel(friend.score)} · ${friend.score}`;
      const bar = document.createElement("i");
      bar.style.setProperty("--value", `${clamp((friend.score / 120) * 100, 0, 100)}%`);
      info.append(name, meta, bar);
      const playdate = document.createElement("button");
      playdate.className = "friend-playdate";
      playdate.type = "button";
      playdate.textContent = careText("playdate");
      playdate.title = `${careText("playdate")} · ${friend.character.name}`;
      playdate.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        performPlaydate(pet, friend.pet);
      });

      const actions = document.createElement("div");
      actions.className = "friend-actions";
      const duoList = document.createElement("div");
      duoList.className = "friend-duo-list";
      for (const moveId of DUO_MOVE_IDS) {
        const move = DUO_MOVES[moveId];
        const readiness = duoMoveReady(pet, friend.pet, move);
        const duo = document.createElement("button");
        duo.className = "friend-duo";
        duo.classList.toggle("is-locked", !readiness.ok);
        duo.type = "button";
        duo.textContent = `${move.icon} ${careText(move.labelKey)}`;
        duo.title = readiness.ok
          ? `${careText(move.descKey)} · ${duoRewardText(move.reward)}`
          : `${careText(move.labelKey)} · ${
              readiness.reason === "energy"
                ? `${careText("duoNeedEnergy")} -${move.energyCost}`
                : `${careText("friendClose")} ${move.minFriendship}`
            }`;
        duo.addEventListener("click", (event) => {
          event.preventDefault();
          event.stopPropagation();
          performDuoMove(pet, friend.pet, moveId);
        });
        duoList.appendChild(duo);
      }
      actions.append(playdate, duoList);
...
function renderPetSynergyPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const activeCount = totalPetSynergies(pet, care, game);
  const friend = bestSynergyFriend(pet);
  const card = document.createElement("section");
  card.className = "synergy-card";

  const head = document.createElement("div");
  head.className = "synergy-head";
  const title = document.createElement("strong");
  title.textContent = careText("petSynergyTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("petSynergyActive")} ${activeCount}/${PET_SYNERGY_IDS.length}`;
  head.append(title, count);

  const friendLine = document.createElement("div");
  friendLine.className = "synergy-friend";
  if (friend) {
    const label = document.createElement("em");
    label.textContent = careText("petSynergyFriend");
    const name = document.createElement("strong");
    name.textContent = `${friend.character.name} · ${friendshipLabel(friend.score)} ${friend.score}`;
    friendLine.append(label, name);
  } else {
    friendLine.classList.add("is-empty");
    friendLine.textContent = careText("petSynergyNoFriend");
  }

  const grid = document.createElement("div");
  grid.className = "synergy-grid";
  for (const synergyId of PET_SYNERGY_IDS) {
    const synergy = { id: synergyId, ...PET_SYNERGIES[synergyId] };
    const unlocked = petSynergyUnlocked(pet, care, game, synergy);
    const item = document.createElement("article");
    item.className = "synergy-item";
    item.classList.toggle("is-locked", !unlocked);
    item.style.setProperty("--synergy-color", synergy.color);
    ...
    const name = document.createElement("strong");
    name.textContent = careText(synergy.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(synergy.descKey);
    const need = document.createElement("em");
    need.textContent = unlocked
      ? careText("done")
      : `${careText("petSynergyNeed")} ${petSynergyRequirementText(pet, care, game, synergy)}`;
    const effect = document.createElement("em");
    effect.textContent = `${careText("petSynergyEffect")} ${petSynergyEffectText(synergy)}`;
    info.append(name, desc, need, effect);
  ...
  card.append(head, friendLine, grid);
  return card;
}

function renderLifeStoryPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const current = currentLifeStoryChapter(pet, care, game);
  const unlockedCount = totalLifeStoryChapters(pet, care, game);
  const card = document.createElement("section");
  card.className = "story-card";
  card.style.setProperty("--story-color", current.color || "#42d7c5");

  const head = document.createElement("div");
  head.className = "story-head";
  const title = document.createElement("strong");
  title.textContent = careText("lifeStoryTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("lifeStoryUnlocked")} ${unlockedCount}/${LIFE_STORY_CHAPTER_IDS.length}`;
  head.append(title, count);
  ...
  const name = document.createElement("strong");
  name.textContent = `${careText("lifeStoryCurrent")} · ${careText(current.labelKey)}`;
  const desc = document.createElement("span");
  desc.textContent = careText(current.descKey);
  info.append(name, desc);
  main.append(icon, info);

  const grid = document.createElement("div");
  grid.className = "story-grid";
  for (const chapterId of LIFE_STORY_CHAPTER_IDS) {
    const chapter = { id: chapterId, ...LIFE_STORY_CHAPTERS[chapterId] };
    const unlocked = lifeStoryChapterUnlocked(pet, care, game, chapter);
    const item = document.createElement("article");
    item.className = "story-item";
    item.classList.toggle("is-locked", !unlocked);
    item.style.setProperty("--story-color", chapter.color);

    const chapterIcon = document.createElement("span");
    chapterIcon.className = "story-item__icon";
    chapterIcon.textContent = chapter.icon;

    const chapterInfo = document.createElement("div");
    chapterInfo.className = "story-item__info";
    const chapterName = document.createElement("strong");
    chapterName.textContent = careText(chapter.labelKey);
    const chapterDesc = document.createElement("span");
    chapterDesc.textContent = careText(chapter.descKey);
    const need = document.createElement("em");
    need.textContent = unlocked
      ? careText("done")
      : `${careText("lifeStoryNeed")} ${lifeStoryRequirementText(pet, care, game, chapter)}`;
    chapterInfo.append(chapterName, chapterDesc, need);
    item.append(chapterIcon, chapterInfo);
    grid.appendChild(item);
  }
...
function renderPackPanel(pet) {
  ...
  const title = document.createElement("strong");
  title.textContent = careText("packTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("packTimes")} ${totalPackEvents(care)}`;
  head.append(title, count);

  const memberBar = document.createElement("div");
  memberBar.className = "pack-members";
  const previewMembers = activePackMembers(pet, PACK_EVENTS.pixelParade).slice(0, 4);
  if (previewMembers.length < 2) {
    const empty = document.createElement("span");
    empty.className = "pack-member is-empty";
    empty.textContent = careText("packNeedMembers");
    memberBar.appendChild(empty);
  } else {
    for (const member of previewMembers) {
      const character = characterFor(member.characterId);
      const chip = document.createElement("span");
      chip.className = "pack-member";
  ...
  for (const eventId of PACK_EVENT_IDS) {
    const packEvent = PACK_EVENTS[eventId];
    const readiness = packEventReady(pet, packEvent);
    const runs = packEventCount(care, eventId);
    const button = document.createElement("button");
    button.className = "pack-button";
    button.classList.toggle("is-locked", !readiness.ok);
    button.type = "button";
    button.style.setProperty("--pack-color", packEvent.color);
    button.title = readiness.ok
      ? `${careText(packEvent.descKey)} · ${packRewardText(packEvent.reward)}`
      : readiness.reason === "members"
        ? `${careText("packNeedMembers")} ${readiness.members.length}/${packEvent.minMembers}`
        : `${careText("packNeedEnergy")} -${packEvent.energyCost} ${careText("energy")}`;

    const icon = document.createElement("span");
    icon.className = "pack-icon";
    icon.textContent = packEvent.icon;

    const info = document.createElement("span");
    info.className = "pack-info";
    const name = document.createElement("strong");
    name.textContent = careText(packEvent.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(packEvent.descKey);
    const meta = document.createElement("em");
    meta.textContent = readiness.ok
      ? `${careText("packMembers")} ${readiness.members.length}/${packEvent.minMembers} · -${packEvent.energyCost} ${careText("energy")} · ${careText("packTimes")} ${runs}`
      : readiness.reason === "members"
        ? `${careText("packNeedMembers")} ${readiness.members.length}/${packEvent.minMembers}`
        : `${careText("packNeedEnergy")} · -${packEvent.energyCost} ${careText("energy")}`;
    const reward = document.createElement("em");
    reward.textContent = `${careText("packReward")} ${packRewardText(packEvent.reward)}`;
    info.append(name, desc, meta, reward);

    const action = document.createElement("span");
    action.className = "pack-action";
    action.textContent = readiness.ok
      ? `+ ${careText("packRun")}`
      : readiness.reason === "members"
        ? careText("packNeedMembers")
        : careText("packNeedEnergy");

    button.append(icon, info, action);
    button.addEventListener("click", (clickEvent) => {
      clickEvent.preventDefault();
      clickEvent.stopPropagation();
      performPackEvent(pet, eventId);
    });
    grid.appendChild(button);
  }

  card.append(head, memberBar, grid);
  return card;
}

function renderContestPanel(pet) {
  const care = careForPet(pet);
  const card = document.createElement("section");
  ...
  const title = document.createElement("strong");
  title.textContent = careText("contestTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("packTimes")} ${totalContestRuns(care)}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "contest-grid";
  for (const contestId of PET_CONTEST_IDS) {
    const contest = PET_CONTESTS[contestId];
    const readiness = contestReady(pet, contest);
    const projected = contestScoreFor(care, contest);
    const projectedTier = contestTierFor(projected);
    const best = contestBestScore(care, contestId);
    const runs = contestRunCount(care, contestId);
    const reward = contestRewardFor(contest, projected, projectedTier);
    const button = document.createElement("button");
    button.className = "contest-button";
    button.classList.toggle("is-locked", !readiness.ok);
    button.type = "button";
    button.style.setProperty("--contest-color", contest.color);
    button.title = readiness.ok
      ? `${careText(contest.descKey)} · ${careText("contestScore")} ${projected} · ${packRewardText(reward)}`
      : readiness.reason === "level"
        ? `${careText("contestNeedLevel")} ${contest.minLevel}`
        : `${careText("contestNeedEnergy")} -${contest.energyCost} ${careText("energy")}`;

    const icon = document.createElement("span");
    icon.className = "contest-icon";
    icon.textContent = contest.icon;

    const info = document.createElement("span");
    info.className = "contest-info";
    const name = document.createElement("strong");
    name.textContent = careText(contest.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(contest.descKey);
    const meta = document.createElement("em");
    meta.textContent = readiness.ok
      ? `${careText("contestScore")} ${projected} · ${careText(projectedTier.key)} · -${contest.energyCost} ${careText("energy")}`
      : readiness.reason === "level"
        ? `${careText("contestNeedLevel")} ${contest.minLevel}`
        : `${careText("contestNeedEnergy")} · -${contest.energyCost} ${careText("energy")}`;
    const bestLine = document.createElement("em");
    bestLine.textContent = `${careText("contestBest")} ${best} · ${careText("packTimes")} ${runs}`;
    info.append(name, desc, meta, bestLine);

    const action = document.createElement("span");
    action.className = "contest-action";
    action.textContent = readiness.ok
      ? `+ ${careText("contestEnter")}`
      : readiness.reason === "level"
        ? careText("contestNeedLevel")
        : careText("contestNeedEnergy");

    button.append(icon, info, action);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      performContest(pet, contestId);
    });
    grid.appendChild(button);
  }
...
function renderLeagueSeasonPanel(pet) {
  const game = ensureGame();
  const season = leagueSeasonFor(game);
  const finalTarget = LEAGUE_SEASON_TIERS[LEAGUE_SEASON_TIERS.length - 1]?.target || 1;
  const claimed = new Set(season.claimedTiers);
  const card = document.createElement("section");
  card.className = "league-season-card";

  const head = document.createElement("div");
  head.className = "league-season-head";
  const title = document.createElement("strong");
  title.textContent = careText("leagueSeasonTitle");
  const count = document.createElement("span");
  count.textContent = `${season.points}/${finalTarget}`;
  head.append(title, count);

  const summary = document.createElement("div");
  summary.className = "league-season-summary";
  const meter = document.createElement("i");
  meter.style.setProperty("--value", `${clamp((season.points / finalTarget) * 100, 0, 100)}%`);
  const facts = document.createElement("div");
  facts.className = "league-season-facts";
  const points = document.createElement("span");
  points.textContent = `${careText("leagueSeasonPoints")} ${season.points}`;
  const best = document.createElement("span");
  best.textContent = `${careText("leagueSeasonBest")} ${season.bestPoints}`;
  facts.append(points, best);
  summary.append(meter, facts);

  const grid = document.createElement("div");
  grid.className = "league-season-grid";
  for (const tier of LEAGUE_SEASON_TIERS) {
    const isClaimed = claimed.has(tier.id);
    const ready = season.points >= tier.target;
    const button = document.createElement("button");
    button.className = "league-season-button";
    button.classList.toggle("is-claimed", isClaimed);
    button.classList.toggle("is-ready", ready && !isClaimed);
    button.classList.toggle("is-locked", !ready);
    button.type = "button";
    button.title = `${careText(tier.labelKey)} · ${tier.target} · ${leagueSeasonRewardText(tier.reward)}`;

    const icon = document.createElement("span");
    icon.className = "league-season-icon";
    icon.textContent = tier.icon;

    const info = document.createElement("span");
    info.className = "league-season-info";
    const name = document.createElement("strong");
    name.textContent = careText(tier.labelKey);
    const meta = document.createElement("span");
    meta.textContent = `${careText("leagueSeasonPoints")} ${Math.min(season.points, tier.target)}/${tier.target}`;
    const reward = document.createElement("em");
    reward.textContent = `${careText("leagueSeasonReward")} ${leagueSeasonRewardText(tier.reward)}`;
    info.append(name, meta, reward);

    const action = document.createElement("span");
    action.className = "league-season-action";
    action.textContent = isClaimed
      ? careText("leagueSeasonClaimed")
      : ready
        ? `+ ${careText("leagueSeasonClaim")}`
        : careText("leagueSeasonLocked");

    button.append(icon, info, action);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      claimLeagueSeasonTier(pet, tier.id);
    });
    grid.appendChild(button);
  }

  card.append(head, summary, grid);
  return card;
}

function renderLeagueMedalPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const equipped = medalForCare(care, game);
  const card = document.createElement("section");
  card.className = "league-medal-card";

  const head = document.createElement("div");
  head.className = "league-medal-head";
  const title = document.createElement("strong");
  title.textContent = careText("medalTitle");
  const count = document.createElement("span");
  count.textContent = `${unlockedMedalCount(care, game)}/${LEAGUE_MEDAL_IDS.length}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "league-medal-grid";

  const none = document.createElement("button");
  none.className = "league-medal-button is-none";
  none.classList.toggle("is-equipped", !equipped);
  none.type = "button";
  none.style.setProperty("--medal-color", "#d1d5db");
  const noneIcon = document.createElement("span");
  noneIcon.className = "league-medal-icon";
  noneIcon.textContent = "--";
  const noneInfo = document.createElement("span");
  noneInfo.className = "league-medal-info";
  const noneName = document.createElement("strong");
  noneName.textContent = careText("medalNone");
  const noneMeta = document.createElement("span");
  noneMeta.textContent = careText("medalEquipped");
  noneInfo.append(noneName, noneMeta);
  const noneAction = document.createElement("span");
  noneAction.className = "league-medal-action";
  noneAction.textContent = !equipped ? careText("medalEquipped") : careText("medalEquip");
  none.append(noneIcon, noneInfo, noneAction);
  none.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    equipLeagueMedal(pet, "");
  });
  grid.appendChild(none);

  for (const medalId of LEAGUE_MEDAL_IDS) {
    const medal = LEAGUE_MEDALS[medalId];
    const unlocked = medalUnlocked(care, game, medal);
    const isEquipped = equipped?.id === medalId;
    const button = document.createElement("button");
    button.className = "league-medal-button";
    button.classList.toggle("is-locked", !unlocked);
    button.classList.toggle("is-equipped", isEquipped);
    button.type = "button";
    button.style.setProperty("--medal-color", medal.color);
    button.title = `${careText(medal.labelKey)} · ${medalRequirementText(care, game, medal)}`;

    const icon = document.createElement("span");
    icon.className = "league-medal-icon";
    icon.textContent = medal.icon;

    const info = document.createElement("span");
    info.className = "league-medal-info";
    const name = document.createElement("strong");
    name.textContent = careText(medal.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(medal.descKey);
    const meta = document.createElement("em");
    meta.textContent = medalRequirementText(care, game, medal);
    info.append(name, desc, meta);

    const action = document.createElement("span");
    action.className = "league-medal-action";
    action.textContent = isEquipped
      ? careText("medalEquipped")
      : unlocked
        ? careText("medalEquip")
        : careText("medalLocked");

    button.append(icon, info, action);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      equipLeagueMedal(pet, medalId);
    });
    grid.appendChild(button);
  }
...
function renderMedalTrialPanel(pet) {
  const care = careForPet(pet);
  const game = ensureGame();
  const medal = medalForCare(care, game);
  const reward = medal ? medalTrialReward(care, medal) : null;
  const energyCost = Math.round(clamp(medal?.trial?.energyCost || 0, 0, 24));
  const ready = !!medal && care.energy >= energyCost;
  const card = document.createElement("section");
  card.className = "medal-trial-card";
  card.classList.toggle("is-empty", !medal);

  const head = document.createElement("div");
  head.className = "medal-trial-head";
  const title = document.createElement("strong");
  title.textContent = careText("medalTrialTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("medalTrialTimes")} ${totalMedalTrials(care)}`;
  head.append(title, count);

  const body = document.createElement("div");
  body.className = "medal-trial-body";
  const icon = document.createElement("span");
  icon.className = "medal-trial-icon";
  icon.style.setProperty("--medal-color", medal?.color || "#d1d5db");
  icon.textContent = medal?.icon || "MD";
  const info = document.createElement("div");
  info.className = "medal-trial-info";
  const name = document.createElement("strong");
  name.textContent = medal ? careText(medal.labelKey) : careText("medalTrialNeedMedal");
  const desc = document.createElement("span");
  desc.textContent = medal ? careText(medal.descKey) : careText("medalTrialNeedMedal");
  info.append(name, desc);
  body.append(icon, info);

  const facts = document.createElement("div");
  facts.className = "medal-trial-facts";
  const factItems = medal
    ? [
        { label: careText("medalTrialEnergy"), value: `${care.energy}/${energyCost}` },
        { label: careText("medalTrialPerk"), value: medalPerkText(medal) },
        { label: careText("medalTrialReward"), value: medalTrialRewardText(reward) },
        { label: careText("medalTrialTimes"), value: `${medalTrialCount(care, medal.id)}` },
      ]
    : [{ label: careText("medalTrialPerk"), value: careText("medalTrialNeedMedal") }];
  ...
  const button = document.createElement("button");
  button.className = "medal-trial-button";
  button.classList.toggle("is-locked", !ready);
  button.type = "button";
  button.textContent = !medal
    ? careText("medalTrialNeedMedal")
    : ready
      ? `+ ${careText("medalTrialRun")}`
      : careText("medalTrialTired");
  button.title = medal
    ? `${careText(medal.labelKey)} · ${medalTrialRewardText(reward)}`
    : careText("medalTrialNeedMedal");
  button.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    performMedalTrial(pet);
  });
...
function renderCollectionPanel() {
  ...
  const title = document.createElement("strong");
  title.textContent = careText("collectionTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("collectionFound")} ${collectionUniqueCount(game)}/${DISCOVERY_ITEM_IDS.length}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "collection-grid";
  for (const id of DISCOVERY_ITEM_IDS) {
    const item = DISCOVERY_ITEMS[id];
    const owned = Math.round(clamp(game.collections?.[id] || 0, 0, 9999));
    const cell = document.createElement("div");
    cell.className = "collection-item";
    cell.classList.toggle("is-locked", owned <= 0);
    cell.dataset.rarity = item.rarity;

    const icon = document.createElement("span");
    icon.className = "collection-icon";
    icon.textContent = owned > 0 ? item.icon : "?";

    const info = document.createElement("span");
    info.className = "collection-info";
    const name = document.createElement("strong");
    name.textContent = owned > 0 ? discoveryLabel(item) : "???";
    const meta = document.createElement("span");
    meta.textContent = owned > 0 ? `${rarityText(item.rarity)} · x${owned}` : careText("locked");
    info.append(name, meta);
    cell.append(icon, info);
    grid.appendChild(cell);
  }

  if (collectionTotalCount(game) === 0) {
    const empty = document.createElement("p");
    empty.className = "collection-empty";
    empty.textContent = careText("collectionEmpty");
    card.append(head, empty, grid);
    return card;
  }
...
function renderCharmPanel(pet) {
  ...
  const head = document.createElement("div");
  head.className = "charm-head";
  const title = document.createElement("strong");
  title.textContent = careText("charmWorkshopTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("charmOwned")} ${inventory.length}/${CHARM_IDS.length}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "charm-grid";
  for (const charmId of CHARM_IDS) {
    const charm = CHARMS[charmId];
    const owned = inventory.includes(charmId);
    const equipped = care.equippedCharm === charmId;
    const craftable = charmCraftable(game, charm);
    const button = document.createElement("button");
    button.className = "charm-button";
    button.type = "button";
    button.classList.toggle("is-owned", owned);
    button.classList.toggle("is-equipped", equipped);
    button.classList.toggle("is-locked", !owned && !craftable);
    button.style.setProperty("--charm-color", charm.color);
    button.title = `${careText(charm.labelKey)} · ${owned ? careText("charmEquip") : charmRecipeText(charm, game)}`;
    ...
    const info = document.createElement("span");
    info.className = "charm-info";
    const name = document.createElement("strong");
    name.textContent = careText(charm.labelKey);
    const desc = document.createElement("span");
    desc.textContent = careText(charm.descKey);
    const recipe = document.createElement("em");
    recipe.textContent = `${careText("charmRecipe")} ${charmRecipeText(charm, game)}`;
    const bonus = document.createElement("em");
    bonus.textContent = `${careText("charmBonus")} ${charmBonusText(charm)}`;
    info.append(name, desc, recipe, bonus);

    const action = document.createElement("span");
    action.className = "charm-action";
    action.textContent = equipped
      ? careText("charmEquipped")
      : owned
        ? careText("charmEquip")
        : craftable
          ? `+ ${careText("charmCraft")}`
          : careText("charmNeedItems");

    button.append(icon, info, action);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();
      craftOrEquipCharm(pet, charmId);
    });
    grid.appendChild(button);
  }
...
function renderPetAlbumPanel() {
  const game = ensureGame();
  const dex = petAlbumFor(game);
  const seenCount = petAlbumSeenCount(game);
  const card = document.createElement("section");
  card.className = "pet-album-card";

  const head = document.createElement("div");
  head.className = "pet-album-head";
  const title = document.createElement("strong");
  title.textContent = careText("petAlbumTitle");
  const count = document.createElement("span");
  count.textContent = `${careText("petAlbumSeen")} ${seenCount}/${PET_ALBUM_IDS.length}`;
  head.append(title, count);

  const grid = document.createElement("div");
  grid.className = "pet-album-grid";
  for (const characterId of PET_ALBUM_IDS) {
    const character = characterFor(characterId);
    const record = dex[characterId] || null;
    const activeSlot = (settings?.slots || []).find((slot) => slot?.enabled === true && slot.character === characterId);
    const activeCare = activeSlot ? normalizeCare(activeSlot.care) : null;
    const seen = !!record || !!activeSlot;
    const bestLevel = Math.max(record?.bestLevel || 0, activeCare?.level || 0);
    const favorites = (FAVORITE_SNACKS[characterId] || FAVORITE_SNACKS.custom || [])
      .map((snackId) => careText(SNACKS[snackId]?.labelKey || snackId))
      .join(", ");

    const item = document.createElement("article");
    item.className = "pet-album-item";
    item.classList.toggle("is-locked", !seen);
    item.classList.toggle("is-active", !!activeSlot);

    const icon = document.createElement("span");
    icon.className = "pet-album-icon";
    icon.textContent = seen ? character.name.slice(0, 2).toUpperCase() : "?";

    const info = document.createElement("div");
    info.className = "pet-album-info";
    const name = document.createElement("strong");
    name.textContent = seen ? character.name : careText("petAlbumHidden");
    const meta = document.createElement("span");
    meta.textContent = seen
      ? `${careText("petAlbumLevel")} ${Math.max(1, bestLevel)} · ${activeSlot ? careText("petAlbumActive") : careText("petAlbumSeen")}`
      : careText("petAlbumHint");
    const likes = document.createElement("em");
    likes.textContent = seen ? `${careText("petAlbumFavorite")} ${favorites}` : careText("locked");
    info.append(name, meta, likes);
    item.append(icon, info);
    grid.appendChild(item);
  }
...
function renderMemoryPanel(pet) {
  const care = careForPet(pet);
  const card = document.createElement("section");
  ...
  title.textContent = careText("memoriesTitle");

  const list = document.createElement("div");
  list.className = "memory-list";
  const memories = Array.isArray(care.memories) ? care.memories.slice(0, 5) : [];
  if (!memories.length) {
    const empty = document.createElement("p");
    empty.className = "memory-empty";
    empty.textContent = careText("emptyMemories");
    list.appendChild(empty);
  } else {
