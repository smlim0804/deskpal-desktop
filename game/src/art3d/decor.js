...
export function stoneWallLow(seed = 1, opt = {}) {
  ...
  for (let i = 0; i < n; i++) {
    const t = i / (n - 1);
    const w = (span / n) * rand(rng, 0.88, 1.02);
    box(m, {
      x: -span / 2 + (span / n) * (i + 0.5),
      w,
      d: rand(rng, 0.34, 0.44),
      h: (0.34 - t * 0.13) * rand(rng, 0.92, 1.08), // 오른쪽으로 갈수록 낮아진다
      color: i % 2 ? STONE : STONE_D,
      top: STONE_TOP,
      ry: rand(rng, -0.12, 0.12),
    });
  }
...
export function stoneWallCoursed(seed = 1, opt = {}) {
  ...
  courses.forEach((c, ci) => {
    ...
    for (let i = 0; i < c.n; i++) {
      box(m, {
        x: -span / 2 + step * (i + 0.5) + rand(rng, -0.03, 0.03),
        y,
        w: step * rand(rng, 0.86, 0.98),
        d: c.d * rand(rng, 0.92, 1.06),
        h: c.h * rand(rng, 0.94, 1.06),
...
export function stoneWallCorner(seed = 1, opt = {}) {
  ...
  for (let c = 0; c < courses; c++) {
    const y = c * ch;
    const h = ch * rand(rng, 0.96, 1.05);
    // 켜마다 모서리를 차지하는 팔이 바뀐다 — 실제 귀돌(quoin) 쌓기 방식
    const flip = c % 2 === 0;
    const ax0 = flip ? 0 : T;
    const az0 = flip ? T : 0;
    // +x 로 뻗는 팔 — 긴 쪽은 돌 두 장으로 나눠 줄눈을 만든다
    const split = ax0 + (RUN - ax0) * rand(rng, 0.42, 0.58);
    const xSeg = [
      [ax0, split],
      [split, RUN],
    ];
...
// ── 지형 시트 : 바위 ────────────────────────────
/** 바위 하나 — 지형 시트: 각진 덩어리 + 발치 자갈 3개 */
export function boulder(seed = 1, opt = {}) {
  const rng = makeRng(seed);
  const m = mesh();
  const r = opt.r || rand(rng, 0.45, 0.72);
  rock(m, { r, h: r * rand(rng, 1.05, 1.4), sides: 6, seed: seed + 1, color: STONE, flat: 0.58 });
  for (let i = 0; i < 3; i++) {
    const a = (i / 3) * Math.PI * 2 + rng();
    rock(m, {
      x: Math.cos(a) * r * 1.25,
      z: Math.sin(a) * r * 1.15,
      r: r * rand(rng, 0.18, 0.3),
...
/** 선돌 무리 — 지형 시트 오른쪽: 키 다른 각기둥 3개 + 발치 잔돌 5개 */
export function standingStones(seed = 1, opt = {}) {
  const rng = makeRng(seed);
  const m = mesh();
  const cols = [
    { x: -0.62, z: 0.1, h: 1.5, r: 0.29 },
    { x: 0.06, z: -0.14, h: 2.35, r: 0.36 },
    { x: 0.74, z: 0.16, h: 1.15, r: 0.3 },
  ];
  for (let i = 0; i < cols.length; i++) {
    ...
    rock(m, {
      ...
      color: STONE,
      topColor: STONE_TOP,
      flat: rand(rng, 0.82, 0.95), // 위로 갈수록 살짝 좁아진다
    });
  }
  for (let i = 0; i < 5; i++) {
    const a = (i / 5) * Math.PI * 2 + 0.4;
    rock(m, {
      x: Math.cos(a) * rand(rng, 0.75, 1.25),
      z: Math.sin(a) * rand(rng, 0.35, 0.6),
      r: rand(rng, 0.15, 0.25),
      h: rand(rng, 0.16, 0.26),
      sides: 5,
      seed: seed + 40 + i,
      color: STONE_D,
    });
  }
  return finish(m, { radius: 1.1, kind: 'standingstones' });
}
...
export function grassMound(seed = 1, opt = {}) {
  ...
  const h = r * rand(rng, 0.3, 0.4);
  dome(m, { r, rz2: r * rand(rng, 0.78, 0.95), h, seg: 9, rings: 2, color: P.grass, wob: 0.06, seed: seed + 1 });
  for (let i = 0; i < 6; i++) {
    const a = (i / 6) * Math.PI * 2 + rand(rng, -0.3, 0.3);
    const rr = r * rand(rng, 0.55, 0.95);
    const y = Math.sqrt(Math.max(0, 1 - (rr / r) * (rr / r))) * h;
    tuft(m, { x: Math.cos(a) * rr, z: Math.sin(a) * rr, y: y - 0.02, h: 0.24, n: 2, seed: seed + 10 + i, spread: 0.07 });
  }
...
export function dirtMound(seed = 1, opt = {}) {
  ...
  const h = r * rand(rng, 0.3, 0.42);
  dome(m, { r, rz2: r * rand(rng, 0.7, 0.9), h, seg: 8, rings: 2, color: P.dirt, wob: 0.09, seed: seed + 1 });
  for (let i = 0; i < 3; i++) {
    const a = (i / 3) * Math.PI * 2 + rand(rng, -0.4, 0.4);
    const rr = r * rand(rng, 0.3, 0.7);
    const y = Math.sqrt(Math.max(0, 1 - (rr / r) * (rr / r))) * h;
    tuft(m, { x: Math.cos(a) * rr, z: Math.sin(a) * rr, y: y - 0.02, h: 0.2, n: 2, seed: seed + 20 + i, spread: 0.06 });
  }
  return finish(m, { radius: r, kind: 'dirtmound' });
}

// ── 지형 시트 : 동굴 · 절벽 · 물가 ───────────────
/** 동굴 입구 — 지형 시트: 큼직한 돌덩이로 짠 아치 + 시커먼 구멍 + 발치 돌부스러기 */
export function caveEntrance(seed = 1, opt = {}) {
  const rng = makeRng(seed);
  const m = mesh();
  const R = 1.0; // 구멍 반너비
  // 안쪽 아치 — 구멍을 따라 도는 돌 7덩이
  const segs = 7;
  for (let i = 0; i < segs; i++) {
    const a = Math.PI * ((i + 0.5) / segs);
    rock(m, {
      x: Math.cos(Math.PI - a) * (R + 0.3),
      y: Math.max(0, Math.sin(a) * (R + 0.5) - 0.22),
      z: 0,
      r: rand(rng, 0.32, 0.42),
      h: rand(rng, 0.42, 0.56),
      sides: 5,
      seed: seed + i,
      color: STONE,
      flat: 0.78,
  ...
  shoulder.forEach((s, i) => {
    rock(m, { x: s[0], y: s[1], z: rand(rng, -0.12, 0.08), r: s[2], h: rand(rng, 0.46, 0.66), sides: 5, seed: seed + 30 + i, color: STONE_D });
  });
  // 시커먼 구멍 — 반원 아치를 그대로 채운 어두운 다각형
  const dark = mesh();
  const pts = [];
  for (let i = 0; i <= 10; i++) {
    const a = Math.PI * (i / 10);
    pts.push([Math.cos(Math.PI - a) * R, Math.max(0, Math.sin(a) * R * 1.4), -0.28]);
  }
  poly(dark, pts, '#3c3833', { double: true });
  merge(m, dark, {});
  // 발치 돌부스러기
  for (let i = 0; i < 3; i++) {
    rock(m, {
      x: rand(rng, -1.2, 1.2),
      z: rand(rng, 0.3, 0.6),
      r: rand(rng, 0.15, 0.24),
      h: rand(rng, 0.12, 0.22),
      sides: 5,
      seed: seed + 60 + i,
      color: STONE_D,
    });
  }
  return finish(m, { radius: 1.6, kind: 'cave' });
}
...
export function cliffChunk(seed = 1, opt = {}) {
  ...
  const D = opt.d || 1.3;
  const H = opt.h || 1.5;
  // 아래가 좁아지는 흙덩이 — topScale > 1 이라 그림처럼 위가 처마처럼 튀어나온다.
  // 평면 점은 반드시 각도가 커지는 순서(반시계)여야 옆면 노멀이 바깥을 본다.
  const plan = [
    [0.5, 0.42],
    [0.1, 0.56],
    [-0.5, 0.5],
  ...
  const pts = plan.map((p) => [p[0] * W * rand(rng, 0.94, 1.0), p[1] * D * rand(rng, 0.94, 1.0)]);
  // 잘린 면은 수직으로 세운다(topScale 1) — 그래야 세로 결을 면에 딱 붙일 수 있다
  chunk(m, { pts, h: H, color: '#c9a978', topColor: '#b3925f', topScale: 1, soft: false });
  // 풀 뚜껑 — 흙보다 조금 넓어 그림처럼 처마같이 걸쳐진다
  const K = 1.08;
  chunk(m, { y: H, pts: pts.map((p) => [p[0] * K, p[1] * K]), h: 0.22, color: P.grassDeep, topColor: P.grass, topScale: 0.98 });
  // 앞쪽 잘린 면의 세로 결 — 얇은 판이 그대로 펜 선이 된다
  const frontZ = Math.max(...pts.map((p) => p[1]));
  for (let i = 0; i < 5; i++) {
    const t = i / 4;
    panel(m, {
      x: (-0.3 + 0.6 * t) * W + rand(rng, -0.04, 0.04),
      y: 0.02,
      z: frontZ + 0.015,
      w: 0.03,
      h: H * rand(rng, 0.55, 0.85),
      color: '#a8875a',
    });
  }
  // 윗면 가장자리를 삐죽삐죽하게 만드는 풀 — 실루엣 밖으로 조금 나가야 그림처럼 보인다
  for (let i = 0; i < 9; i++) {
    const t = i / 9;
    const p0 = pts[Math.floor(t * pts.length) % pts.length];
    const p1 = pts[(Math.floor(t * pts.length) + 1) % pts.length];
    const f = (t * pts.length) % 1;
    tuft(m, {
      x: (p0[0] + (p1[0] - p0[0]) * f) * K * 1.02,
      z: (p0[1] + (p1[1] - p0[1]) * f) * K * 1.02,
      y: H + 0.2,
      h: 0.3,
      n: 2,
      seed: seed + 10 + i,
...
export function steppingStone(seed = 1, opt = {}) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    rock(m, {
      x: t * 2.2,
      z: Math.sin(t * 4.2) * 0.3 + rand(rng, -0.06, 0.06),
      r: rand(rng, 0.24, 0.34),
      h: rand(rng, 0.07, 0.11),
      sides: 6,
      seed: seed + i,
      color: STONE,
      topColor: STONE_TOP,
  ...
  return finish(m, { radius: 1.1, kind: 'stepstone' });
}

/** 돌 두른 웅덩이 — 지형 시트: 둥근 물면 + 두르는 돌 10개 + 부들 + 풀포기 */
export function pondRimRocks(seed = 1, opt = {}) {
  const rng = makeRng(seed);
  const m = mesh();
  const R = opt.r || 1.15;
  waterDisc(m, { y: 0.05, r: R - 0.16, seg: 11, color: WATER });
  for (let i = 0; i < 10; i++) {
    const a = (i / 10) * Math.PI * 2 + rand(rng, -0.1, 0.1);
    rock(m, {
      x: Math.cos(a) * R,
      z: Math.sin(a) * R * 0.86,
      r: rand(rng, 0.2, 0.3),
      h: rand(rng, 0.13, 0.22),
      sides: 5,
      seed: seed + i,
      color: i % 2 ? STONE : STONE_D,
      flat: 0.7,
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const h = rand(rng, 0.5, 0.72);
    const st = mesh();
    tube(st, { r: 0.016, h, seg: 3, color: P.leafDark, cap: false });
    merge(m, st, { rz: lean, tx: rx + i * 0.08, ty: 0.06, tz: rz + i * 0.05 });
    tube(m, { x: rx + i * 0.08 + lean * h, y: 0.06 + h, z: rz + i * 0.05, r: 0.03, h: 0.14, seg: 4, color: '#a8804f' });
  }
